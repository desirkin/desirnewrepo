...
test('K11. UNIVERSE, EARS, WIDE EYE, coin, auth, CLEAR and VETO all use the shared shell', () => {
  ...
  assert.equal(opens.length, 5); // exactly one inside showCard + one inside showLedger + one inside showResearch (MARKET-LAB read-only view) + one inside showJudge (JUDGE read-only explanation drawer) + one inside showSenses (SERPENT paper: read-only sensor / readiness panel)
});
