...
test('K11. UNIVERSE, EARS, WIDE EYE, coin, auth, CLEAR and VETO all use the shared shell', () => {
  ...
  assert.equal(opens.length, 6); // exactly one inside showCard + one inside showLedger + one inside showResearch (MARKET-LAB read-only view) + one inside showAsk (ASK SERPENT read-only conversation drawer) + one inside showJudge (JUDGE read-only explanation drawer) + one inside showSenses (SERPENT paper: read-only sensor / readiness panel)
});
