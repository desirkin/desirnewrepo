...
const RECIPE_DIGEST = sealShadowRecipe(RECIPE).recipeDigest;
const mappedSource = (dir, subject) => ({ dir, venue: subject.venue, canonicalCoin: subject.canonicalCoin, marketIdentityDigest: marketIdentityDigest(subject) });
const catalogSourceFor = (markets, now) => () => {
  const knownAtTs = now() - 1;
  return sealAcceptedCatalogSnapshot({ observedTs: knownAtTs, knownAtTs, maxAgeMs: 10 * MIN, markets });
};
...
test('THROUGH-HORIZON COVERAGE (review P0): a contiguous prefix that never reaches horizonEnd can NEVER pretend a horizon outcome — no-exit and limit-never-filled claims require coverage through horizonEnd; an exit that genuinely occurred inside the prefix still matures', () => {
  ...
  const limit = buildShadowCapture({ recipe: { ...RECIPE, recipeVersion: 'shadow-recipe-limit-1', variants: [{ variantId: 'take-limit', decision: 'TAKE', sizeTier: 'S', intendedAllocation: { kind: 'QUOTE_NOTIONAL', quoteCurrency: 'USD', amount: 600 }, entryRule: 'LIMIT_AT_TRIGGER', limitOffsetBps: 500, exitRule: 'STOP_TARGET_OR_HORIZON', stopPct: 1, targetPct: 1 }, RECIPE.variants[1]] }, venue: 'kraken', assetId: 'BTC', decisionTs: opp.decisionTs, inputs: opp.inputs }).eligible.find((c) => c.variantId === 'take-limit');
...
test('RUNNER rotation under REAL pacing: 3 sources with a 2-bundle step budget starve nobody — the durable round-robin resumes where it left off, each step runs ONE merged batch (no pacing self-collision), and every market is captured across steps', () => {
  ...
  try {
    ...
    const sources = [
      mappedSource(mkBundleFor('b-btc', KRAKEN_BTC, 'KRAKEN_SPOT'), KRAKEN_BTC),
      mappedSource(mkBundleFor('b-eth', COINBASE_ETH, 'COINBASE_SPOT'), COINBASE_ETH),
      mappedSource(mkBundleFor('b-sol', SOL, 'KRAKEN_SPOT'), SOL),
    ];
    let now = T - 4 * MIN + 400; let mono = 0;
    const store = createShadowStore({ dataDir: dir, clock: () => now });
    const runner = createShadowRunner({ store, recipe: RECIPE, acceptedCatalogSource: catalogSourceFor([KRAKEN_BTC, COINBASE_ETH, SOL], () => now), bundleSource: () => sources, maxBundlesPerStep: 2, quotas: { minInterBatchMs: 100 }, clock: () => now, monotonic: () => mono });
    mono = 1000;
    ...
    assert.ok(store.lastControl(`${ROTATION_CONTROL}:${RECIPE.recipeVersion}:${RECIPE_DIGEST}`), 'the rotation cursor is durable');
    assert.ok(store.lastControl(`${CURSOR_CONTROL}:${marketIdentityDigest(KRAKEN_BTC)}:${RECIPE.recipeVersion}:${RECIPE_DIGEST}`), 'per-market telemetry cursors are durable and recipe-bound');
    store.close();
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
...
test('COMPOSITE CURSOR regression: two markets, each with several SAME-RECEIPT windows, maxBatch 1 — every window of every market is captured exactly once across steps AND a mid-way restart; the scalar-clock drop is gone', () => {
  ...
  try {
    ...
    const sources = [
      mappedSource(writeBundle(path.join(dir, 'b-btc'), rowsFor(KRAKEN_BTC), covFor2(KRAKEN_BTC, 'KRAKEN_SPOT')), KRAKEN_BTC),
      mappedSource(writeBundle(path.join(dir, 'b-eth'), rowsFor(COINBASE_ETH), covFor2(COINBASE_ETH, 'COINBASE_SPOT')), COINBASE_ETH),
    ];
    ...
    const mkRunner = (store) => createShadowRunner({ store, recipe: RECIPE, acceptedCatalogSource: catalogSourceFor([KRAKEN_BTC, COINBASE_ETH], () => now), bundleSource: () => sources, quotas: { maxBatch: 1, minInterBatchMs: 1 }, clock: () => now, monotonic: () => (mono += 500) });
    ...
    const cursor = store2.lastControl(`${CURSOR_CONTROL}:${marketIdentityDigest(KRAKEN_BTC)}:${RECIPE.recipeVersion}:${RECIPE_DIGEST}`);
...
test('CROSS-SEGMENT outcomes (review item 5): a path split across two adjacent sealed segments is UNMATURABLE only PROVISIONALLY — retriable, never terminal — and matures once the segments are read together; a second bundle never overwrites the first', () => {
  ...
  try {
    ...
    let now = T - 4 * MIN + 400; let mono = 0;
    const store = createShadowStore({ dataDir: dir, clock: () => now });
    let visible = [mappedSource(b1, KRAKEN_BTC)];
    const runner = createShadowRunner({ store, recipe: RECIPE, acceptedCatalogSource: catalogSourceFor([KRAKEN_BTC], () => now), bundleSource: () => visible, maxBundlesPerStep: 2, quotas: { minInterBatchMs: 1 }, clock: () => now, monotonic: () => (mono += 500) });
    const r1 = runner.step({ nowTs: now });
    ...
    visible = [mappedSource(b1, KRAKEN_BTC), mappedSource(b2, KRAKEN_BTC)];
