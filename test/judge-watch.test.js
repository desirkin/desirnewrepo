...
async function rig({ accountId = 'w-acct', script, controls = { kill: false, cage: false }, falsifiers = () => [], policy, exitPolicy = null } = {}) {
  ...
  const ctl = { ...controls }; const watch = createWatch({ accountId, dispatcher, adapter, feed, clock: pclock, specOf: () => SPEC, feeOf: () => TAKER_FEE, controls: () => ctl, falsifiers, policy: policy ?? WATCH_REFERENCE, exitPolicy });
...
test('W01/W09. the first (partial) fill becomes tracked risk before the entry is terminal: a shell with a pin exists before dispatch, R is PROVISIONAL until the IOC is terminal then FINAL from fills; a 50% fill scales the target proceeds to the ACTUAL residual; first-fill time is immutable; safety work outranks candidate work', async () => {
  // the planned-target counterfactual is opt-in (exitPolicy) — under it the target proceeds are scaled to the actual residual;
  // the LIVE law (default, no exitPolicy) computes no target basis (asserted in W06).
  const r = await rig({ exitPolicy: { plannedTarget: true } }); const pos = await r.enter({ q: '0.002', asks: [[99990.0, 0.001]] }); assert.equal(pos.state, 'OPEN'); assert.equal(pos.confirmedBase, '0.001', 'half filled'); assert.equal(r.state().orders.o1.state, 'EXPIRED', 'IOC remainder is terminal'); assert.ok(pos.firstFillTs); assert.equal(pos.feedPinned, true);
  assert.equal(pos.initialR.state, 'FINAL', 'entry terminal + fills reconciled -> FINAL R'); assert.ok(M.isPositive(pos.initialR.value)); assert.equal(pos.initialR.entryVwap, '99990'); const tracked = r.watch.positions()[0]; assert.ok(tracked.targetPerUnit, 'the opt-in counterfactual scales target proceeds per unit to the actual residual'); assert.equal(pos.protection.state, 'ACTIVE'); assert.equal(pos.protection.qty, '0.001');
...
test('W06. the live exit law has NO target: a price spike into the frozen scenario target does NOT exit; the backstops (no-progress at 180s, mandatory 4h duration) still fire; the opt-in counterfactual still exits at the target when enabled; no hidden fixed-return quota anywhere in the policy', async () => {
  ...
  // LIVE LAW (default, no exitPolicy): the bid runs past the scenario target and the position does NOT take a planned target;
  // it rides (only the stop / trail / thesis-invalidation / backstops can exit it) — no target price or percent anywhere.
  const live = await rig({ accountId: 'w-notarget' }); await live.enter({ stop: '99500', target: '102000' }); live.clock.advance(1000); live.book([[102400.0, 5]], [[102390.0, 5], [102380.0, 5]]); await live.settle(); await live.settle();
  assert.notEqual(live.watch.positions()[0].exit.reason, 'PLANNED_TARGET', 'the live law never takes a planned target'); assert.equal(live.state().positions.p1.state, 'OPEN', 'it rides past the scenario target'); assert.equal(live.watch.positions()[0].targetPerUnit, null, 'no target basis is computed in the live law');
  // the retained counterfactual (opt-in): with exitPolicy { plannedTarget: true } the planned full exit still works — this is the D4 experiment's 'with target' arm.
  const t = await rig({ accountId: 'w-target', exitPolicy: { plannedTarget: true } }); await t.enter({ stop: '99500', target: '102000' }); t.clock.advance(1000); t.book([[102400.0, 5]], [[102390.0, 5], [102380.0, 5]]); await t.settle(); await t.settle(); const w = t.watch.positions()[0]; assert.equal(w.exit.reason, 'PLANNED_TARGET', JSON.stringify(t.watch.positions()[0])); assert.equal(w.exit.priority, 'P4_TRAIL_OR_TARGET'); t.clock.advance(300); t.book([[102400.0, 5]], [[102390.0, 5]]); await t.settle(); await t.advance(600); const pos = t.state().positions.p1; assert.equal(pos.state, 'FLAT'); assert.ok(M.isPositive(pos.realizedPnl), 'net of entry / exit fees');
