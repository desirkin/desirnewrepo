...
const PROVIDERS = ['KRAKEN_SPOT', 'COINBASE_SPOT', 'KRAKEN_DERIVATIVES', 'COINGECKO', 'DEFILLAMA', 'SETTLED_RECORDS'];
...
test('SENSE-CULL-2 fence: KEPT_MARKET_PROVIDERS is the closed roster of senses-page market providers and equals the owner factories; retired and owner-only providers are absent', () => {
  ...
  // SENSE-CULL-3 fence: PROVIDER_IDS is EXACTLY the kept market providers plus the owner-only SETTLED_RECORDS projection —
  // no registered provider exists outside that closed set (a provider added to PROVIDER_IDS but not KEPT fails here).
  assert.deepEqual([...PROVIDER_IDS].sort(), [...new Set([...KEPT_MARKET_PROVIDERS, 'SETTLED_RECORDS'])].sort(), 'PROVIDER_IDS === KEPT_MARKET_PROVIDERS ∪ {SETTLED_RECORDS}');
  assert.equal(KEPT_MARKET_PROVIDERS.length, 11, 'eleven kept market providers after SENSE-CULL-3');
  // The retired providers are gone from every roster.
  for (const gone of ['FRED', 'COINMETRICS', 'TWELVEDATA', 'DERIBIT', 'BYBIT', 'SANTIMENT', 'BINANCE_SPOT']) { assert.ok(!KEPT_MARKET_PROVIDERS.includes(gone), `${gone} retired from KEPT`); assert.ok(!MARKET_FACTORY_IDS.includes(gone), `${gone} retired from the factories`); assert.ok(!PROVIDER_IDS.includes(gone), `${gone} retired from PROVIDER_IDS`); }
});
...
test('A12/B06. INTEGRATED owner: the Tape observer seam copies accepted trades / books into bounded hot state (stopped owner ignores, a full queue drops with DROPPED coverage, an observer exception is contained); the deep-market adapter yields a lawful v1 window from the owner\'s ACTUAL retained windows and the REAL Social strainer consumer builds a dossier whose marketDeep features come from that window', async () => {
  ...
  try {
    ...
    assert.equal(validateDeepMarketWindow(input).ok, true); assert.equal(input.windowStartTs, base); assert.equal(input.windowEndTs, base + 60_000); assert.equal(input.state, 'SYNCHRONIZED');
    // closeout R02/W05: the integrated seam carries accepted trades but NO positive subscription-continuity fact for the interval, so the
    // interval trade aggregate is WITHHELD (trades: null, TRADE_COVERAGE_UNKNOWN) while the accepted book still supports the window
    assert.equal(input.trades, null, 'observed trades without complete interval coverage are never an interval total'); assert.equal(explain.coverage.withheld, 'TRADE_COVERAGE_UNKNOWN'); assert.equal(explain.coverage.support, 'UNKNOWN_COVERAGE'); assert.ok(explain.coverage.observedCount >= 20); assert.equal(input.priceStart, null); assert.equal(input.book.source, 'kraken-ws-v2-accepted-book');
    ...
    assert.equal(d.dossier.marketDeep.state, 'SYNCHRONIZED', 'the real consumer sees the adapter window');
    assert.equal(d.dossier.marketDeep.features.flow.count, null, 'trades withheld (no positive interval coverage) reach the consumer as NOT SUPPLIED, never as a total'); assert.equal(d.dossier.marketDeep.features.flow.netTakerNotionalUsd, null); assert.equal(d.dossier.marketDeep.features.priceChangePct, null); assert.ok(d.dossier.marketDeep.features.book.spreadBps > 0, 'the accepted book still supports the window'); assert.equal(d.dossier.executability.state, 'ASSESSED'); assert.ok(windows >= 1); assert.equal(rt.status().deepMarket, 'INJECTED_ADAPTER');
