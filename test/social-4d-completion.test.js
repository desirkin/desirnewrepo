...
if (!TEST_URL) {
  ...
  test('Q (PG). the collector validates Social history on restore with the provider gate OFF; corrupt Social history withholds fail-closed before any provider is enabled; a clean mixed history restores and, once enabled, hydrates the same history', async () => {
    await withDb(async ({ mkStores }) => {
      ...
      const boot = ({ checkpointStore, journal, social = {} }) => { seed(); const clock = { ms: NOW }; const c = startRumor2({ log: () => {}, config: { universe: ['BTC', 'ETH', 'SOL'] }, fetchImpl: async () => mkRes(304, ''), now: () => clock.ms, intervalMs: 2_147_000_000, checkpointStore, journal, contact: 'ops@example.com', enabled: true, timeoutMs: 5000, ...social }); return { c, tick: async () => ((clock.ms += 4_000_000), await c.tickOnce()) }; };
