...
test('RC-1. DATA_ONLY composes the starters in the published order with the published options, then reports ACTIVE', async () => {
  ...
  try {
    ...
    assert.deepEqual(f.calls.map((c) => c.name), [
      ...
      'startDataOnlyMarket', 'startWideEye', '_refreshCatalog', '_sweepOnce', 'startBroadKraken', 'startGateway', 'startRumor2', 'tickOnce',
    ]);
    ...
    // LEAN PASS 4a: the public-discovery collector (GDELT / Polymarket / Kalshi) is retired — not composed.
    assert.equal(f.calls.find((c) => c.name === 'startPublicDiscovery'), undefined);
    ...
    assert.deepEqual(f.stops, ['rumor2', 'gateway', 'broadMarket', 'wideEye', 'market', 'checkpoints', 'persistence'], 'collectors stop in reverse start order, then the checkpoints close, then persistence stops');
...
test('RC-3. durable restore unavailable → fail closed: no quota-bearing collector is constructed, the governor refuses, RUMOR-2 is withheld, status says why', async () => {
  ...
  try {
    ...
    assert.equal(status.blockers.YOUTUBE, undefined, 'LEAN PASS 4a: no YouTube collector, no YOUTUBE blocker'); assert.equal(status.blockers.PUBLIC_DISCOVERY, undefined, 'LEAN PASS 4a: no discovery collector, no PUBLIC_DISCOVERY blocker'); assert.match(status.blockers.RUMOR2, /durable PostgreSQL restore or quota ownership unavailable/);
