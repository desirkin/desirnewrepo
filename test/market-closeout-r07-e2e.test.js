...
import { T0, tmp, json, btcOnly, includedPlan, cryptoquantPolicy, cryptoquantFetch, krakenBookOnly, bookObs, krakenWsScript, krakenTradeMsg, krakenRestFixture, H, SEALED_REF } from './helpers/market-closeout.js';
...
test('MC-RD03 (readiness). an explicitly partial family keeps the overall state non-green; a QUALIFIED synthetic all-required case passes READINESS_GREEN (the gate is not hardwired red); the old count-only form (null provider, endpoint e, no metrics) is non-green with named missing proof', () => {
  ...
  const partial = liveReadinessManifest({ rows, familyCoverage: { ...full, DERIVATIVES_FUNDING_OI: qualified('DERIVATIVES_FUNDING_OI', { complete: false, support: { state: 'PARTIAL', basis: 'CENSUS_RECORD' } }) }, modelReadiness: model, generatedTs: T0 });
  assert.equal(partial.families.DERIVATIVES_FUNDING_OI.state, 'PARTIAL_LIVE'); assert.notEqual(partial.overall, 'READINESS_GREEN'); assert.ok(partial.familiesNotLive.includes('DERIVATIVES_FUNDING_OI')); assert.ok(partial.blockers.some((b) => /DERIVATIVES_FUNDING_OI/.test(b)));
...
test('MC-RD04 (readiness). declared-policy counts are computed from the 15-family artifact; a disabled or unauthorised alternative provider never overrides the qualified source', () => {
  const PUBLIC = ['KRAKEN_SPOT', 'COINBASE_SPOT', 'KRAKEN_DERIVATIVES', 'COINGECKO', 'GECKOTERMINAL', 'DEFILLAMA', 'SETTLED_RECORDS']; const subjects = btcOnly();
  const count = (m) => { const c = {}; for (const v of Object.values(m.families)) c[v.state] = (c[v.state] ?? 0) + 1; return c; };
  const pub = buildCoverageMatrix({ policy: loadPolicy(H.policyWith({ providers: PUBLIC })), subjects, env: {} }); assert.equal(Object.keys(pub.families).length, 15); assert.deepEqual(Object.keys(pub.families), [...FAMILIES]);
  const pc = count(pub); assert.equal(pc.COVERED_BY_DECLARED_POLICY + (pc.ENABLED_BUT_BLOCKED ?? 0) + (pc.UNCOVERED ?? 0), 15, 'the counts partition the 15 families'); assert.equal(pub.families.ONCHAIN_ENTITY_FLOW.state, 'UNCOVERED', 'a paid-only family is UNCOVERED under a public policy');
  ...
  // CryptoQuant is now the SOLE route for ONCHAIN_ENTITY_FLOW (SENSE-CULL-3 retired the Santiment alternative): the qualified source stands alone.
  assert.deepEqual(authorised.families.ONCHAIN_ENTITY_FLOW.routes.map((r) => r.providerId), ['CRYPTOQUANT']);
  // Attesting CryptoQuant covers BOTH ONCHAIN_ENTITY_FLOW and NETWORK_ACTIVITY: since SENSE-CULL-2 retired Coin Metrics and
  // SENSE-CULL-3 retired Santiment, CryptoQuant is the sole provider of both, so a public policy leaves both UNCOVERED until attested.
...
// ---------------------------------------------------------------- E01-E07 --------------------------------------------------------
// the connected fixture world: Kraken REST + WS (positive trade coverage), CryptoQuant network metric
async function connectedOwner({ clock, dir, extra = {} }) {
  ...
  const raw = H.policyWith({ providers: ['KRAKEN_SPOT', 'CRYPTOQUANT'], model: MODEL }); includedPlan(raw, 'CRYPTOQUANT', extra.plan ?? {}); const policy = loadPolicy(raw); const wire = [];
  const fetchImpl = async (url, init) => { const u = new URL(url); wire.push(u.host + u.pathname); if (u.host === 'api.kraken.com') return H.fetchFor(http)(url, init); if (u.host === 'api.cryptoquant.com') return cryptoquantFetch()(url); return json({}, 404); };
...
async function populatedCase({ dir, clk, script, extra = {} }) {
  ...
  await owner.start({ outDir: path.join(dir, 'cap'), families: ['NETWORK_ACTIVITY'] }); await H.waitFor(() => owner.status().streams.KRAKEN_SPOT?.subscribed?.length >= 1, { timeoutMs: 5000 });
...
test('MC-E01 (joined). production owner + fake HTTP/WS -> guarded acquisition -> sealed capture / prefix -> context -> v2 packet -> ACTUAL Anthropic client over a scripted response -> validated report -> sealed case -> reopen; the serialized model request carries the expected numbers, support states and citations', async () => {
  ...
  try {
    ...
    assert.ok(wire.some((w) => w.includes('addresses-count')), `guarded acquisitions happened (${JSON.stringify([...new Set(wire)])})`);
    ...
    for (const needle of ['MARKET_DISPLAYED_EXIT_LIQUIDITY_CHANGE', String(exit.value.current.lossBps), 'MARKET_NETWORK_CONTEXT', '7000', 'MARKET_CHART_WINDOW', '"COMPLETE"', 'KRAKEN_SPOT', 'CRYPTOQUANT']) assert.ok(text.includes(needle), `the serialized request carries ${needle}`);
...
test('MC-E04 (joined). partial truth stays visible from context to packet to case: book-only startup and a paid access refusal are the correct limitations; a demonstrated quiet interval is a valid zero; observations available never masquerade as a complete interval', async () => {
  const dir = tmp(); const paths = []; const raw = H.policyWith({ providers: ['CRYPTOQUANT', 'KRAKEN_SPOT'] });
  const owner = createResearchOwner({ policy: loadPolicy(raw), subjects: btcOnly(), env: {}, clock: () => T0, researchRoot: dir, log: () => {}, fetchImpl: async (url) => { const u = new URL(url); paths.push(u.host + u.pathname); if (u.host === 'api.kraken.com') return json(u.pathname.endsWith('AssetPairs') ? H.KRAKEN_ASSET_PAIRS : H.krakenTicker('XXBTZUSD')); return json({}, 404); } });
  try {
    ...
    const net = await owner.acquire('NETWORK_ACTIVITY', 'BTC'); assert.ok(!paths.some((p) => p.includes('cryptoquant')), 'no key => no paid dispatch'); assert.ok(net.usage.reasons.CREDENTIAL_MISSING >= 1);
    const quietCov = makeCoverage({ provider: 'KRAKEN_SPOT', endpointId: 'ws-v2', subjectId: subjectId(market.subject), family: 'SPOT_FLOW', kind: 'TRADE', state: 'SUBSCRIBED', reasonCodes: [], startTs: T0 - 600_000, endTs: null, observationCount: 0, droppedCount: 0, epochId: null, sequenceStart: null, sequenceEnd: null }); const observations = [...tick.observations]; const coverage = [...tick.coverage, ...net.coverage, quietCov];
    const ctx = buildContext({ canonicalCoin: 'BTC', asOfTs: T0, observations, coverage, captureRef: REF, referenceNotionals: [100] }).context; assert.equal(contextError(ctx), null);
    ...
    assert.equal(ev('MARKET_NETWORK_CONTEXT').state, 'UNAVAILABLE'); assert.equal(ev('MARKET_CHART_WINDOW').value.support.state, 'COMPLETE_NO_TRADES'); assert.equal(ev('MARKET_CHART_WINDOW').value.fields.count, 0);
    ...
    assert.equal(r.status, 'BUDGET_BLOCKED'); const sealedPacket = JSON.parse(readFileSync(path.join(dir, 'case', 'packets.jsonl'), 'utf8').trim()); assert.equal(sealedPacket.evidence.find((e) => e.kind === 'MARKET_NETWORK_CONTEXT').state, 'UNAVAILABLE', 'the limitation survives into the sealed case');
...
test('MC-E05 (joined). a recording failure while a model request is active: the service stops healthy admission, cancels and drains ownership, preserves the source prefix / error and accounts the in-flight charge conservatively; no late healthy case', async () => {
  ...
  // registry grows (was 50k at earlier provider counts; a reference-only sense like BITSTAMP_SPOT still enlarges the sealed policy).
...
test('MC-E06 (joined). restart / reopen: with every process cache gone the capture, context, packets and case reopen and verify (prefix, ids, values, coverage, report); a replay with the recorded model output makes zero network calls; reusing the account journal never resets accounting', async () => {
  ...
  const cm = JSON.parse(readFileSync(path.join(caseDir, 'case.json'), 'utf8')); const seg = cm.inputs[0].captureRef.segments[0]; const cap = readCapture(seg.dir); assert.equal(cap.bundle.manifest.bundleId, seg.bundleId); assert.ok(cap.observations.some((o) => o.kind === 'TRADE') && cap.coverage.some((c) => c.state === 'SUBSCRIBED'));
