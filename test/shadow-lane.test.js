...
const RECIPE = {
  ...
  variants: [
    ...
    { variantId: 'take-open-l', decision: 'TAKE', sizeTier: 'L', entryRule: 'NEXT_CANDLE_OPEN', limitOffsetBps: null, stopPct: 1, targetPct: 1 },
...
test('fidelity honesty: candle-only outcomes can NEVER carry size evidence (large-size variants included); a depth-supported capture matured with an observed depth path records DEPTH_SUPPORTED_OBSERVED', () => {
  const built = buildShadowCapture({ recipe: RECIPE, venue: 'kraken', assetId: 'BTC', decisionTs: T, inputs: { candles: WINDOW } });
  ...
  assert.equal(od.sizeEvidence, 'DEPTH_SUPPORTED_OBSERVED');
});
