...
function makeFakeDb() {
  ...
  function run(body, p) {
    ...
    if (body.startsWith(RESULT_INSERT_BULK_PREFIX)) { const n = p.length / 10; for (let i = 0; i < n; i++) { const b = i * 10; t.result.push({ identity: p[b], day_key: p[b + 1], batch_id: p[b + 2], row_ordinal: p[b + 3], sim_id: p[b + 4], status: p[b + 5], completed: p[b + 6], valid_modeled: p[b + 7], prospective_eligible: p[b + 8] }); } return { rows: [], rowCount: n }; }
    ...
    switch (B2T.get(body)) {
      ...
      case SQL.BATCH_INSERT: { const k = bk(p[0], p[1], p[2]); if (t.batch.has(k)) throw new Error('dup batch'); t.batch.set(k, { identity: p[0], day_key: p[1], batch_id: p[2], job_id: p[3], payload_digest: p[5], resulting_revision: p[7], cursor_before: p[8], next_cursor: p[9], done: p[10], evidence_digest: p[14] }); return { rowCount: 1, rows: [] }; }
      ...
      case SQL.COMPLETED_LIST: { const out = []; for (const k of t.completed.keys()) { const [id, d, sim] = k.split('|'); if (id === p[0] && d === p[1]) out.push(sim); } out.sort(); const lim = p[2] ?? out.length; return { rows: out.slice(0, lim).map((sim_id) => ({ sim_id })) }; }
      case SQL.COMPLETED_COUNT: { let n = 0; for (const k of t.completed.keys()) { const [id, d] = k.split('|'); if (id === p[0] && d === p[1]) n++; } return { rows: [{ n }] }; }
      case SQL.PENDING_LIST: { const out = []; for (const [k, v] of t.pending) { const [id, d, sim] = k.split('|'); if (id === p[0] && d === p[1]) out.push({ sim_id: sim, ...v }); } const lim = p[2] ?? out.length; return { rows: out.slice(0, lim) }; }
      ...
      case SQL.BATCH_LIST_DAY: { const out = []; for (const v of t.batch.values()) if (v.identity === p[0] && v.day_key === p[1]) out.push({ batch_id: v.batch_id, job_id: v.job_id, next_cursor: v.next_cursor, cursor_before: v.cursor_before, payload_digest: v.payload_digest, done: v.done, resulting_revision: v.resulting_revision }); out.sort((a, b) => a.resulting_revision - b.resulting_revision); const lim = p[2] ?? out.length; return { rows: out.slice(0, lim).map(({ batch_id, job_id, next_cursor, cursor_before, payload_digest, done }) => ({ batch_id, job_id, next_cursor, cursor_before, payload_digest, done })) }; }
