...
import { orderKeyOf, createL3Book } from '../market-lab/l3-book.js';
...
test('PT-03. the capture runner: both senses under a policy that enables them, every dispatch through the accounting guard, one sealed EDGE_CAPTURE bundle (its own layout), reopened under the same contracts with dark kinds only; provisional / final buckets and L3 records land where declared; the status is IMPLEMENTED_DARK_NOT_EVALUATED with every authority NONE; a run with no dark sense enabled is refused; REPLAY mode never fetches', async () => {
  ...
  try {
    ...
    assert.equal(r.charts.failures, 0); assert.deepEqual(r.charts.symbols, ['PF_XBTUSD', 'PF_ETHUSD']); assert.equal(r.counters.analytics, 2 * 6 * 3); assert.equal(r.l3.verdict, 'SAFE_L3_DATA_KEY'); assert.equal(r.l3.blocker, null); assert.equal(r.l3.started, true); assert.equal(r.capture, 'COMPLETE'); assert.equal(r.counters.rejected, 0); assert.deepEqual(r.l3.markets, ['BTC/USD', 'ETH/USD']); assert.equal(r.counters.snapshots >= 3, true); assert.equal(r.counters.events, 1);
    ...
    assert.equal(edge.bundle.manifest.identity.status, 'IMPLEMENTED_DARK_NOT_EVALUATED'); assert.equal(edge.bundle.manifest.identity.authority, 'NONE'); assert.equal(edge.bundle.manifest.summary.l3.verdict, 'SAFE_L3_DATA_KEY'); assert.equal(edge.bundle.manifest.summary.capture, 'COMPLETE'); assert.equal(edge.bundle.manifest.summary.rejected, 0); assert.equal(edge.capture, 'COMPLETE'); assert.ok(!JSON.stringify(edge.bundle.manifest).includes('TOKEN-VALUE'));
    ...
    const spot = edge.observations.filter((o) => o.kind === 'L3_ORDER_EVENT')[0]; assert.deepEqual(spot.payload.events.map((e) => e.event), ['MODIFY']); assert.equal(spot.payload.events[0].previousQty, 0.88968699); assert.equal(spot.payload.checksumVerified, true); assert.equal(spot.sourceEventTs, Date.parse('2026-09-10T12:00:00.500Z'), 'W10: the exchange clock of the update survives the sealed bundle beside receivedTs'); assert.notEqual(spot.sourceEventTs, spot.receivedTs);
