...
test('SCOPE-56 blockers. a scope row whose inventory rows are ALL NOT_PRESENT names an exact blocker and dated route evidence; the publisher rows name the frozen-core rule conflict; every NOT_PRESENT row is composed nowhere, paid nowhere and holds no authority', () => {
  ...
  assert.deepEqual(absent, ['I01', 'I02', 'I03', 'P01', 'P02', 'P03', 'P04', 'P05', 'P06', 'P07', 'P08', 'P09', 'S10'], 'the requested capabilities the repository does not implement — accounted for, not hidden');
  for (const j of joined) for (const r of j.rows) if (r.classification === 'NOT_PRESENT') {
    ...
    if (j.id.startsWith('P')) { assert.match(r.readinessBlocker, /doctrine\/RUMOR2\.md/, `${j.id}: the rule conflict is named, not hidden`); assert.ok(INVENTORY_GROUPS.includes('PUBLISHER_NEWS') && r.group === 'PUBLISHER_NEWS'); }
  }
