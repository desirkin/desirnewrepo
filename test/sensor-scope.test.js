...
test('SCOPE-46. exactly the 36 stable ids (M01-M08, M11-M13, M16-M17, M20, S01/S02/S03/S05/S09, N01-N06, P05-P08, I05-I08, C01-C03), unique, each mapped to at least one EXISTING inventory row (LEAN PASS 4a retired the I01-I03 infra tier, the S10 social-video/YouTube tier, the I04 Tally governance sense, the non-crypto publishers P01-P04 / P09 with their aggregator + licensed machinery, and the S04 official StockTwits, S06 TikTok and S07/S08 Meta routes with the social-official cut; the market-lab providers cut retired the M19 Twelve Data cross-asset market sense with the whole CROSS_ASSET family; SENSE-CULL-2 retired the M14 Coin Metrics Community and M15 FRED / ALFRED market senses to attic; SENSE-CULL-3 retired the M09 Deribit, M10 Bybit and M18 Santiment market senses to attic)', () => {
  assert.equal(SCOPE_VERSION, 'serpent-sensor-scope-1'); assert.equal(SCOPE.length, 36); assert.equal(new Set(SCOPE_IDS).size, 36);
  const expected = [...Array.from({ length: 20 }, (_, i) => `M${String(i + 1).padStart(2, '0')}`).filter((m) => !['M09', 'M10', 'M14', 'M15', 'M18', 'M19'].includes(m)), 'S01', 'S02', 'S03', 'S05', 'S09', ...Array.from({ length: 6 }, (_, i) => `N${String(i + 1).padStart(2, '0')}`), ...Array.from({ length: 4 }, (_, i) => `P${String(i + 5).padStart(2, '0')}`), ...Array.from({ length: 4 }, (_, i) => `I${String(i + 5).padStart(2, '0')}`), ...Array.from({ length: 3 }, (_, i) => `C${String(i + 1).padStart(2, '0')}`)];
  ...
  const joined = scopeInventory(inv); assert.equal(joined.length, 36); assert.ok(joined.every((j) => j.rows.every((r) => r !== null)));
});

test('SCOPE-56 blockers. a scope row whose inventory rows are ALL NOT_PRESENT names an exact blocker and dated route evidence; every NOT_PRESENT row is composed nowhere, paid nowhere and holds no authority (LEAN PASS 4a retired the only two all-NOT_PRESENT scope rows, the Reuters / Bloomberg licensed publishers, so the scope now carries none)', () => {
  ...
  assert.deepEqual(absent, [], 'no scope capability is fully absent: the licensed non-crypto publishers were retired, not left as accounted absences');
  ...
  for (const id of ['P05', 'P06', 'P07', 'P08']) for (const r of joined.find((j) => j.id === id).rows) { assert.notEqual(r.classification, 'NOT_PRESENT', id); assert.match(r.module, /^press\/collector\.js/, id); assert.match(r.compositionPath, /fly\.js -> startPress/, id); assert.equal(r.decisionAuthority, 'NONE'); assert.equal(r.group, 'PUBLISHER_NEWS'); assert.equal(r.paid, false); }
