// SERPENT PAPER — the 53-row requested sensor scope is a checked-in map onto the sensor inventory, fenced: exactly the 53
...
test('SCOPE-53. exactly the 53 stable ids (M01-M20, S01-S10, N01-N06, P01-P09, I04-I08, C01-C03), unique, each mapped to at least one EXISTING inventory row (LEAN PASS 4a retired the I01-I03 infra observation tier)', () => {
  assert.equal(SCOPE_VERSION, 'serpent-sensor-scope-1'); assert.equal(SCOPE.length, 53); assert.equal(new Set(SCOPE_IDS).size, 53);
  ...
  const joined = scopeInventory(inv); assert.equal(joined.length, 53); assert.ok(joined.every((j) => j.rows.every((r) => r !== null)));
});
