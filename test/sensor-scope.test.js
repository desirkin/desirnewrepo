...
test('SCOPE-46. exactly the 46 stable ids (M01-M20, S01-S09, N01-N06, P05-P08, I05-I08, C01-C03), unique, each mapped to at least one EXISTING inventory row (LEAN PASS 4a retired the I01-I03 infra tier, the S10 social-video/YouTube tier, the I04 Tally governance sense, and the non-crypto publishers P01-P04 / P09 with their aggregator + licensed machinery)', () => {
  assert.equal(SCOPE_VERSION, 'serpent-sensor-scope-1'); assert.equal(SCOPE.length, 46); assert.equal(new Set(SCOPE_IDS).size, 46);
  ...
  const joined = scopeInventory(inv); assert.equal(joined.length, 46); assert.ok(joined.every((j) => j.rows.every((r) => r !== null)));
});
