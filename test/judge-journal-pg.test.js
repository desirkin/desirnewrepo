...
test('D01. migration 8 is additive on the established baseline: versions 1-7 untouched, SCHEMA_VERSION 8, the new tables exist, an existing control row is unchanged', { skip }, async () => {
  assert.equal(SCHEMA_VERSION, 9); assert.deepEqual(MIGRATIONS.map((m) => m.version), [1, 2, 3, 4, 5, 6, 7, 8, 9]); /* old law 8; new law 9 = focused-completion experiment records, additive */ assert.equal(MIGRATIONS[6].name, 'RUMOR-2 writer epoch: database fencing token');
  const { rows } = await db.query('SELECT version FROM serpent_schema_migrations ORDER BY version'); assert.deepEqual(rows.map((r) => r.version), [1, 2, 3, 4, 5, 6, 7, 8, 9]); /* old law 8, new law 9 (experiment records) */
