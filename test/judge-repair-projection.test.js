...
const COMPOSED_POLICY = (await import('../judge/policy.js')).loadJudgePolicy(POLICY_FILE); const COMPOSED_INIT = Object.freeze({ policyDigest: COMPOSED_POLICY.digest, policyVersion: COMPOSED_POLICY.policy.policyName });
...
async function heldAccount(accountId, clock) {
  const r = await paperAccount({ accountId, clock, init: COMPOSED_INIT }); const { F } = r;
...
test('R16-01. the paper depletion checkpoint is written on stop (bounded, versioned, account-bound) and a FRESH composition restores it; a checkpoint for another account is ignored', async () => {
  const clock = fakeClock(); const r = await paperAccount({ accountId: 'cp-acct', clock, init: COMPOSED_INIT }); await r.writer.release();
...
test('R16-03. the projection is BOUND to account / run mode / revision: the reader rejects a file for another account, another mode, an unbound file and a revision that went backwards (projecting no posture); the cockpit view rejects a cross-account file while a composition runs; the posture machine names a rejection and projects nothing', async () => {
  ...
  const clock = fakeClock(); const r = await paperAccount({ accountId: 'acct-c', clock, init: COMPOSED_INIT }); await r.writer.release(); const run = await composeJudge({ policyFile: POLICY_FILE, mode: 'PAPER', accountId: 'acct-c', journal: r.journal, clock: pclockOf(clock), specs: [SPEC], nominations: () => [], writeProjection: false, codeDigest: CODE, log: () => {} }); await run.start({ heartbeatMs: 3_600_000 }); setJudgeRun(run);
...
test('R16-05. E09: a late adapter callback after stop() is fenced (no commit, no throw, counted); an unwritable projection directory is a counted, reported failure that never stops the run (checkpoint and projection alike)', async () => {
  ...
  try {
    const r2 = await paperAccount({ accountId: 'nofile-acct', clock, init: COMPOSED_INIT }); await r2.writer.release(); const run2 = await composeJudge({ policyFile: POLICY_FILE, mode: 'PAPER', accountId: 'nofile-acct', journal: r2.journal, clock: pclockOf(clock), specs: [SPEC], nominations: () => [], writeProjection: true, codeDigest: CODE, log: () => {} });
