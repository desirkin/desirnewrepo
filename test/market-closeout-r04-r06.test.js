...
test('MC-B03 (R04). the requested-age boundary is exact on knowledge clocks; a HISTORY interval with one point inside is thin (PARTIAL), two points satisfy; the result coverage carries the SOURCE period, never the receipt; an observation known after Q0 is excluded at Q0 and admitted at Q1', async () => {
  ...
  const two = await history({ windowStartTs: d6 - 1, windowEndTs: d8 }); assert.equal(two.state, 'SATISFIED', JSON.stringify(two.metrics)); assert.equal(two.observationsAdmitted, 2); assert.equal(two.coverage.startTs, d6, 'coverage starts at the SOURCE period, not the receipt'); assert.ok(two.coverage.endTs <= d8 && two.coverage.endTs >= d7); assert.equal(two.coverage.knownAtMax, T0);
  const one = await history({ windowStartTs: d7 + 1, windowEndTs: d8 }); assert.notEqual(one.state, 'SATISFIED', 'one point inside a two-day window is thin'); assert.equal(one.metrics.active_addresses.historyThin, true);
