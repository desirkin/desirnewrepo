...
test('R06-05. gain restrictions are account-specific: the composition derives the lock level from THIS account\'s session P&L against the configured thresholds, never from the legacy ledger\'s daily lock', async () => {
  ...
  // a realized gain of USD 40 on 500 (8%) is above the configured hard lock threshold (cobra.config.json locks); the journal-projection lock (no projection here) knows nothing of it
  ...
  const { dailyLockStatus } = await import('../state/locks.js'); assert.equal(dailyLockStatus().level, 'NONE', 'the journal-projection lock is NONE with no projection and is irrelevant to this account');
  await run.stop();
});
