...
if (!TEST_URL) {
  ...
  test('D1. schema 4 applies; rumint checkpoint round-trips tri-state with revision counting', async () => {
    ...
    try {
      ...
      assert.equal(m.schemaVersion, 9, 'old law 8, new law 9 (focused-completion experiment records, additive): RUMINT-R1 schema (and later) landed (JUDGE execution schema 8 is additive; 1-7 untouched)');
