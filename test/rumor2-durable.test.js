...
if (!TEST_URL) {
  ...
  test('D61-63. migrations apply (schema 6 with the event journal); rumor2 checkpoint round-trips with monotonic revisions', async () => {
    ...
    try {
      ...
      assert.equal(m.schemaVersion, 10, 'laws 1-9 remain unchanged; law 10 adds bounded small-store snapshot anchors: RUMOR-2 event-root schema (and later) landed in Development PostgreSQL');
