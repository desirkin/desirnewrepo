...
test('G01. trades 2 units at 100 and 1 at 102: quantity 3, notional 302, vwap 100.666667; signed notional and known-side imbalance follow the taker side; a coverage gap inside the window nulls the counts instead of understating them', () => {
  const w = tradeWindow([trade(100, 2, 'BUY', T0 - 30_000, 1), trade(102, 1, 'SELL', T0 - 10_000, 2)], { startTs: T0 - 60_000, endTs: T0 });
  ...
  const gap = tradeWindow([trade(100, 2, 'BUY', T0 - 30_000)], { startTs: T0 - 60_000, endTs: T0, evictedUntilTs: T0 - 40_000 }); assert.equal(gap.count, null); assert.equal(gap.quoteNotional, null); assert.equal(gap.support.state, 'PARTIAL'); assert.deepEqual(gap.support.reasons, ['COVERAGE_GAP_INSIDE_WINDOW']);
  const empty = tradeWindow([], { startTs: T0 - 60_000, endTs: T0 }); assert.equal(empty.count, 0); assert.equal(empty.vwap, null); assert.equal(empty.support.state, 'COMPLETE_NO_TRADES', 'no trades in a covered window is an observed zero, not a missing value');
...
test('G07. ATM IV 0.65; call IV 0.70 at delta +0.24; put IV 0.80 at delta -0.26: risk reversal -0.10, butterfly +0.10; nearest strike / delta selection is deterministic; a mixed scope refuses; admission is nearest expiry then nearest strike, capped', () => {
  ...
  const s = optionsSurface(ticks); const t = s.term[0];
  assert.equal(t.atm.markIv, 0.65); assert.equal(t.call25.instrumentId, 'C24'); assert.equal(t.put25.instrumentId, 'P26'); near(t.riskReversal25d, -0.10); near(t.butterfly25d, 0.10); assert.equal(s.law, 'NO_DEALER_GAMMA_INFERENCE_FROM_PUBLIC_OI'); assert.equal(s.support.state, 'COMPLETE');
