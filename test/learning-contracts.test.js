...
import {
  opportunityIdOf, candidateIdOf, activationIdOf, episodeError, patternRecordError, activationError, campaignManifestError,
...
test('activation ceilings: per-activation cap, lifetime cap, allowlisted kind only, lone-loss degradation impossible', () => {
  const a = (over = {}) => {
    const record = {
    activationVersion: ACTIVATION_VERSION, activationId: null, seq: 0, state: 'PUBLISHED_WAITING_FOR_PAPER',
    ...
    applicability: { clauses: [{ feature: 'rv60', op: 'GTE', threshold: 0 }] }, previousVersion: null, effectiveTs: T, expiresTs: T + 86_400_000, cooldownUntilTs: null,
    ...
    record.activationId = activationIdOf({ candidateId: record.candidateId, patternId: record.patternId, candidateDigest: record.candidateDigest, effectiveTs: record.effectiveTs });
    return record;
  };
