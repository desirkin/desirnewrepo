...
const { composeJudge, initAccount } = await import('../judge/composition.js'); const { loadJudgePolicy } = await import('../judge/policy.js'); const { createScriptedWebSocket } = await import('./helpers/scripted-ws.js');
...
test('R15-01. the LIVE venue-owner slot is retained when the shutdown reconciliation is INCOMPLETE and released only after a verified handoff (competing DB sessions)', { skip }, async () => {
  ...
  const compose = (journal) => composeJudge({ policyFile: LIVE_FILE, mode: 'LIVE_UNARMED', accountId: 'pg-live', env: ENV, journal, clock: pclockOf(clock), transport, WebSocketImpl: createScriptedWebSocket().WebSocketImpl, specs: [SPEC], allowPrivate: () => true, allowOrders: () => false, writeProjection: false, codeDigest: 'c'.repeat(64), log: () => {}, nominations: () => [] });
