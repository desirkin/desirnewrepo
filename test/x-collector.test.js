...
function fakeXApi() {
  ...
  const fetchImpl = async (url, opts = {}) => {
    ...
    if (u.pathname === '/2/usage/tweets') return res(200, { data: { project_usage: '10', project_cap: '3000000', daily_project_usage: { project_id: 'fixture', usage: [{ date: iso(T1), usage: '0' }] } } });
...
if (!TEST_URL) {
  ...
  const boot = ({ checkpointStore, journal, clockMs = T1, social = {} }) => {
    ...
    const c = startRumor2({
      ...
      ...social,
      // SOCIAL-4F: the paid X anchors are an EXPLICIT injected static scope here (labelled INJECTED_STATIC) — never config.universe
      socialXOptions: { universe: [...CONFIG.universe], aliases: ['bitcoin'], ...(social.socialXOptions ?? {}) },
    });
