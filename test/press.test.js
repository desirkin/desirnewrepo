// PRESS — the crypto publisher-observation tier (owner scope P05-P08): registry law, pure item mapping, the composed collector
// under an injected transport (valid feed -> observations through the closed shape, 304, 429 with Retry-After, malformed /
// empty feeds, containment of one failed source, restart continuity), the reader, the profile / readiness wiring and the
// composition + authority fences. No network: every transport is injected; the offline guard would refuse otherwise.
// LEAN PASS 4a retired the non-crypto publishers with the AGGREGATOR + LICENSED_INTERFACE_REQUIRED machinery; the retired
// aggregator / licensed test cases are preserved in attic/senses/press/press-aggregator-licensed.test.js.
...
import { itemToObservation, pressObservationError, PRESS_LIMITS } from '../press/parse.js';
...
const rss = (items) => `<?xml version="1.0"?><rss version="2.0"><channel><title>T</title>${items.map((i) => `<item><title>${i.t}</title><link>${i.l}</link><guid>${i.g}</guid><pubDate>${i.p ?? 'Fri, 12 Sep 2026 03:00:00 GMT'}</pubDate><description>${i.d ?? 'sum'}</description></item>`).join('')}</channel></rss>`;
...
test('PRESS-1. registry law: four stable crypto ids, closed PUBLISHER / RSS vocabularies, https feed on the pinned host, cadence floor, dated docs; every mutation is refused', () => {
  assert.equal(pressRegistryError(), null); assert.deepEqual([...PRESS_SOURCE_IDS], ['COINDESK_NEWS', 'THEBLOCK_NEWS', 'COINTELEGRAPH_NEWS', 'DECRYPT_NEWS']);
  assert.deepEqual([...PRESS_KINDS], ['PUBLISHER']); assert.deepEqual([...PRESS_ROUTES], ['RSS']);
  for (const s of PRESS_SOURCES) { assert.equal(s.kind, 'PUBLISHER'); assert.equal(s.route, 'RSS'); assert.ok(s.docs.every((d) => d.accessedOn === '2026-09-12' && /^https:\/\//.test(d.url))); assert.ok(s.feedUrl.startsWith('https://')); assert.equal(new URL(s.feedUrl).hostname, s.host); assert.ok(s.cadenceSec >= 300); }
  assert.equal(pressSource('COINDESK_NEWS').name, 'CoinDesk'); assert.equal(pressSource('NOPE'), null);
  const mut = (fn) => { const list = PRESS_SOURCES.map((s) => ({ ...s, docs: [...s.docs] })); fn(list); return pressRegistryError(list); };
  assert.match(mut((l) => { l[0].feedUrl = 'http://www.coindesk.com/x'; }), /https/); assert.match(mut((l) => { l[0].feedUrl = 'https://evil.example/rss'; }), /host/); assert.match(mut((l) => { l[0].cadenceSec = 30; }), /cadence/); assert.match(mut((l) => { l[1].id = 'COINDESK_NEWS'; }), /duplicate/); assert.match(mut((l) => { l[0].kind = 'PRIMARY'; }), /vocabulary|kind/);
});

test('PRESS-2. pure mapping: headline / link / publisher clock only, bounded, identity from the source + guid (or link), future-dated items skipped; the direct-feed publisher is the source itself; the closed shape refuses body fields and forged ids', () => {
  const src = pressSource('COINDESK_NEWS'); const base = { source: src, receiptTs: T0, feedKind: 'RSS' };
  ...
  assert.equal(pressObservationError(o), null); assert.equal(o.title.length, PRESS_LIMITS.maxTitleChars); assert.equal(o.summary.length, PRESS_LIMITS.maxSummaryChars); assert.equal(o.linkHost, 'www.coindesk.com'); assert.equal(o.sourceKind, 'PUBLISHER'); assert.equal(o.publisher, 'CoinDesk'); assert.equal(o.publisherHost, 'www.coindesk.com'); assert.equal(o.bodyFetched, false); assert.equal(o.authority, 'NONE'); assert.equal(o.knownAtTs, T0); assert.equal(o.coverage, 'HEADLINE_LINK_ONLY');
  ...
  assert.match(pressObservationError({ ...o, body: 'full article' }), /keys|body/i); assert.match(pressObservationError({ ...o, observationId: 'f'.repeat(64) }), /identity|observationId/i); assert.match(pressObservationError({ ...o, bodyFetched: true }), /law|body/i); assert.match(pressObservationError({ ...o, authority: 'NOMINATE' }), /law|authority/i); assert.match(pressObservationError({ ...o, sourceKind: 'AGGREGATOR' }), /source/i);
});

test('PRESS-3. composed collector under an injected transport: dark without PRESS_ENABLED (null, zero calls); only PRESS_SOURCES rows poll; a valid feed appends observations through the closed shape then the checkpoint; 304 / 429 (Retry-After floored) / HTML / empty feeds are distinct states; one failed source never touches another; restart resumes with the etag and seen ids; a second start returns the same handle', async () => {
  ...
  const env = { PRESS_ENABLED: 'true', PRESS_SOURCES: ON }; assert.deepEqual(pressSelectedIds(env), ON.split(','));
  const h = startPress({ env, dataDir: dir, fetchImpl, clock: () => t, log: () => {}, timers, signals: false }); assert.ok(h);
  assert.deepEqual(await h.pollOnce('COINDESK_NEWS'), { outcome: 'OBSERVED', admitted: 2 }); assert.deepEqual(await h.pollOnce('THEBLOCK_NEWS'), { outcome: 'PARSE_FAILED', admitted: 0 }); assert.deepEqual(await h.pollOnce('DECRYPT_NEWS'), { outcome: 'RATE_LIMITED', admitted: 0 }); assert.deepEqual(await h.pollOnce('COINTELEGRAPH_NEWS'), { outcome: 'EMPTY_FEED', admitted: 0 });
  const st = readPressStatus(dir); assert.equal(st.v, 'press-status-1'); assert.equal(st.sources.COINDESK_NEWS.state, 'OBSERVED'); assert.equal(st.sources.THEBLOCK_NEWS.state, 'PARSE_FAILED'); assert.equal(st.sources.DECRYPT_NEWS.state, 'RATE_LIMITED'); assert.equal(st.sources.DECRYPT_NEWS.backoffUntil, t + 60_000, 'Retry-After 5 s floored at 60 s'); assert.equal(st.sources.COINTELEGRAPH_NEWS.state, 'EMPTY_FEED'); assert.equal(st.authority, 'NONE');
  for (const s of Object.values(st.sources)) assert.ok(PRESS_STATES.includes(s.state), s.state);
  assert.ok(calls.every((c) => /^SerpentCobra\/press/.test(c.ua)));
...
test('PRESS-4. reader + snapshot: corrupt / forged lines are counted and never returned; the sensor snapshot colours a publisher ACTIVE only from a FRESH status record, DISABLED for OFF profile rows, BLOCKED_PROVIDER after a stale failure; the profile derives PRESS_ENABLED / PRESS_SOURCES from the ON rows only', () => {
  const p = loadProfile(); assert.ok(PROFILE_GROUPS.includes('publisherNews')); const env = profileEnvironment(p); assert.equal(env.PRESS_ENABLED, 'true'); assert.deepEqual(env.PRESS_SOURCES.split(','), ON.split(','));
  ...
  const s0 = sensorSnapshot({ profile: p, env: { ...env }, dataDir: dir, now }); assert.ok(SNAPSHOT_GROUPS.includes('PUBLISHER_NEWS')); assert.equal(s0.groups.PUBLISHER_NEWS.length, 4);
  assert.equal(snapshotRow(s0, 'PRESS_COINDESK_NEWS').state, 'NOT_OBSERVED'); assert.ok(s0.rows.filter((x) => x.group === 'PUBLISHER_NEWS').every((x) => x.authority === 'NONE'));
  const status = (state, lastSuccessTs, lastError = null) => ({ v: 'press-status-1', tsMs: now, enabled: true, authority: 'NONE', sources: { COINDESK_NEWS: { kind: 'PUBLISHER', route: 'RSS', desired: 'ON', state, cadenceSec: 600, terms: 'UNVERIFIED', coverage: 'HEADLINE_LINK_ONLY', lastReceiptTs: now, lastSuccessTs, lastOutcome: null, lastError, backoffUntil: null, counters: { polls: 1, admitted: 1 } } } });
...
test('Every supported crypto publisher composes to its reader with its own direct publisher identity; replay after a lost checkpoint deduplicates', async () => {
  const dir = tmp('press-all-'), sources = [...PRESS_SOURCES];
  const options = { env: { PRESS_ENABLED: 'true', PRESS_SOURCES: sources.map((s) => s.id).join(',') }, dataDir: dir, clock: () => T0, timers, signals: false, log: () => {}, fetchImpl: async (url) => {
    const source = sources.find((s) => s.feedUrl === url); assert.ok(source); return new Response(rss([{ t: `Fixture ${source.name}`, l: `https://${source.host}/fixture`, g: 'same-guid' }]), { headers: { 'content-type': 'application/rss+xml' } });
  } };
  let h = startPress(options); for (const s of sources) assert.equal((await h.pollOnce(s.id)).admitted, 1); h.stop();
  const rows = readPressObservations(dir).observations; assert.equal(rows.length, 4); assert.equal(new Set(rows.map((o) => o.observationId)).size, 4); assert.equal(rows.find((o) => o.sourceId === 'COINDESK_NEWS').publisher, 'CoinDesk');
  const { unlinkSync } = await import('node:fs'); for (const s of sources) unlinkSync(path.join(dir, 'press', `checkpoint-${s.id}.json`));
  h = startPress(options); for (const s of sources) assert.equal((await h.pollOnce(s.id)).admitted, 0); h.stop(); assert.equal(readPressObservations(dir).observations.length, 4);
});
