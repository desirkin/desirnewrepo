...
test('spot family cache clocks enforce the published effective cadence and never restamp cached observations', async () => {
  ...
  rawSubjects.macroSeries = []; rawSubjects.crossAsset = []; rawSubjects.stablecoins = []; rawSubjects.peers = []; rawSubjects.benchmarks = ['BTC'];
...
test('a malformed provider response is cadence-gated too, so failures cannot restore five-minute request flooding', async () => {
  const policy = loadPolicy(H.policyWith({ providers: ['KRAKEN_SPOT', 'COINBASE_SPOT'] }));
  const rawSubjects = structuredClone(sampleSubjects()); rawSubjects.subjects = [rawSubjects.subjects[0]]; rawSubjects.macroSeries = []; rawSubjects.crossAsset = []; rawSubjects.stablecoins = []; rawSubjects.peers = []; rawSubjects.benchmarks = ['BTC'];
