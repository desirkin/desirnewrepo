...
test('SCOPE-2 (G). collisions never bind: terms inside URLs / @handles, substrings, Unicode lookalikes, operator-injection strings, oversized text; matching is order-independent and deterministic', () => {
  ...
  assert.deepEqual(matched('$ВТС listing'), [], 'Cyrillic lookalikes never fold into BTC'); assert.deepEqual(matched('$BTC​ listing'), ['BTC:CASHTAG'], 'an ASCII cashtag followed by an invisible char is still that cashtag — nothing was folded'); assert.deepEqual(matched('$B​TC listing'), [], 'an invisible char inside the token breaks it, never folds');
