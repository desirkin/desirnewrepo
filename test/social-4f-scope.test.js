...
const REV3_DOCTRINE_LINES = ["", "### Closeout appendix, revision 3 — the four remaining boundaries (A–D)", "", "1. **Readiness reconciles requested and obtained scope.** Complete means the declared requested assets, metrics and", "   count were obtained; a shortfall is a named partial, and a complete claim over a shortfall is a contradiction that never", "   qualifies. A model demonstration counts only once it has occurred by the manifest's own clock.", "2. **One native period is one sample.** Envelope identity is not sample identity: one version per native period of one", "   compatible series is selected under the as-of law before anything is counted, gridded or computed; repeats, revisions and", "   conflicts are disclosed; different providers, venues, quotes, intervals or units never merge.", "3. **A journal append ends with its primary close.** The first write, fsync or close failure surfaces and latches; the", "   descriptor is closed once and never retried; a failed initialization leaves neither descriptor nor lock; a reopen validates", "   the actual journal and never truncates a valid row to regain allowance.", "4. **A component's family is bound to its metric.** Family, metric, recipe and version agree with one closed binding at the", "   builder, the candidate, the reader, the verifier and the packet; hashes, counts and available values authorize nothing.", "", "Authority remains NONE. No profit target, return quota or forced-trade objective belongs in any readiness or acceptance claim."]; // revision 3: the four remaining boundaries appendix (exact lines)
const REV2_DOCTRINE_LINES = ["", "### Closeout appendix, revision 2 — the five remaining laws (P1–P5)", "", "1. **Readiness is qualified, never counted.** A family is LIVE only from evidence that names its selected required", "   provider, a family-relevant registry endpoint, the requested and obtained assets and registered metrics, the measured", "   interval, a knowledge clock fresh under the family's own policy age and a support basis; a historical smoke is a", "   separate fact; the model needs a supported demonstration; missing proof is named, never thrown.", "2. **Accounting commits only what became durable.** A transition is validated, then its bytes are appended, fsynced and", "   closed, then the validated state change is applied; a rejected transition writes nothing, a failed or uncertain write", "   latches the journal and refuses every later admission, a failed reservation never dispatches, a failed settlement is a", "   named failure that admits no data and keeps the reservation charged, and a restart resolves the ambiguous conservatively.", "3. **Stop owns its work.** Admission stops; every in-flight acquisition and request (shared, unshared, coalesced, queued)", "   is tracked apart from the coalescing map, aborted and drained within a bounded deadline; open reservations are preserved", "   as unresolved while the journal is still owned; late continuations are detached; only the admitted prefix is sealed;", "   the journal is released last.", "4. **Support is demonstrated.** A history interval is complete only over the observations' own period grid with no", "   missing period, or over positive coverage records for point observations; a derived candle metric needs its actual", "   warmup over a contiguous series; identities are counted once; a cache hit cannot erase a gap.", "5. **Every container of the saved context is closed.** Membership is own-property membership; scalars are typed;", "   quantities are magnitudes and signed metrics keep their sign; a diagnostic names the path, never the untrusted text.", "", "Authority remains NONE. No readiness claim, acceptance claim, prompt, test or configuration carries a profit target,", "a return quota or a forced-trade objective."];
test('SCOPE-8 (9/10). protected surfaces are byte-identical to 9c17372; authority: the 4F modules import only inside their tier and touch no trading, control, or model surface', () => {
  // LEAN PASS 4a (social-official cut): rumor2/social-stocktwits.js, rumor2/social-meta.js and rumor2/social-tiktok.js were
  // retired to attic/ with the Meta / TikTok / official-StockTwits providers, so they are no longer pinned in the living tree.
  const pinned = ['survey/eyecore.js', 'tape/universe.js', 'tape/run.js', 'cost/model.js', /* ledger/ledger.js retired to attic/ (lean trim step 1, 2026-09-14) */ 'rumor2/truth.js', 'rumor2/social.js', 'rumor2/social-time.js', 'rumor2/social-reconcile.js', 'rumor2/social-view.js', 'rumor2/x-stream.js', 'rumor2/providers/bluesky-official.js', 'rumor2/providers/farcaster-official.js', 'rumor2/providers/x-official.js', 'rumor2/social-reddit.js', 'rumor2/social-farcaster-access.js', 'rumor2/social-foundation.js', 'rumor2/social-registry.js', 'doctrine/MISSION.md', 'doctrine/SOCRATES.md', 'package.json', 'package-lock.json'];
  ...
  const AUTHORIZED_DELTA = {
    ...
    // admits AFTER the tape subscribed has none; requestBookSnapshot re-subscribes THAT symbol's book channel at its own depth (pure
    // bookSnapshotRequest decision, rate-limited, BOOK_SNAPSHOT_REQUESTED event). No wider venue, no deeper book, no new credential —
    // exact line delta, recomputed against 9c17372.
    'tape/run.js': { added: ["            checksumVerified = check.ok;", "            levels: () => ({ bids: [...book.bids.entries()].sort((a, b) => b[0] - a[0]).slice(0, 100).map(([price, qty]) => [price, qty]), asks: [...book.asks.entries()].sort((a, b) => a[0] - b[0]).slice(0, 100).map(([price, qty]) => [price, qty]) }) }));", "          let checksumVerified = null;", "          observe(() => observer.onBook({ coin: coinFromSymbol(d.symbol), symbol: d.symbol, receivedTs: Date.now(), synced: book.synced, checksumVerified, pricePrecision: book.pricePrecision, qtyPrecision: book.qtyPrecision,", "          observe(() => observer.onTrade({ coin: coinFromSymbol(t.symbol), symbol: t.symbol, side: t.side, qty: t.qty, price: t.price, eventTs: Date.parse(t.timestamp), receivedTs: Date.now(), tradeId: t.trade_id ?? null, ordType: t.ord_type ?? null, snapshot: msg.type === 'snapshot' }));", "        .filter((p) => !p.major && !unavailable.has(p.symbol) && !pinned.has(p.symbol)) // held / pending exposure is never shed; caps stay", "      // ONE captured owner clock per snapshot: the appended record, and the passive current", "      // feature file (SOCIAL-5 §36.7 read-only bridge) carry the SAME computed object and instant", "      bookSnapshotAsked.set(symbol, now);", "      books.get(symbol)?.desync(); // the round trip loses updates: the tape's own book is not trusted until the snapshot lands", "      const now = Date.now();", "      const pinned = pinnedSymbols();", "      const r = bookSnapshotRequest({ symbol, pair: pairs.get(symbol) ?? null, unavailable: unavailable.has(symbol), socketOpen: ws?.readyState === WebSocket.OPEN, lastRequestMs: bookSnapshotAsked.get(symbol) ?? null, nowMs: now });", "      const snapshot = { ts: new Date(tsMs).toISOString(), coin: p.coin, tapeState, ...bf, ...flows.get(p.symbol).features(tsMs) };", "      const tsMs = Date.now();", "      feedGuard(() => executionFeed.onConnect(Date.now()));", "      feedGuard(() => executionFeed.onDisconnect(Date.now()));", "      for (const m of r.messages) send(m);", "      if (!incoming.has(symbol) && !pinned.has(symbol)) removed.push(symbol); // a held / pending symbol is never dropped by a volume-rank refresh", "      if (!r.ok) return r;", "      if (entry?.priority === 'HELD' || entry?.priority === 'PENDING') { adoptPair({ coin: coinFromSymbol(symbol), symbol, major: false, depth: entry.depth ?? xp.defaultDepth ?? 25, usdVol24h: null, pinned: true }); if (ws?.readyState === WebSocket.OPEN) subscribeSymbols([symbol]); writeEvent('PAIR_PINNED', { symbol, priority: entry.priority }); return { ok: true, adopted: true }; }", "      if (pairs.has(symbol)) { if (unavailable.has(symbol) && (entry?.priority === 'HELD' || entry?.priority === 'PENDING')) { unavailable.delete(symbol); subFailures.delete(symbol); if (ws?.readyState === WebSocket.OPEN) subscribeSymbols([symbol]); } return { ok: true, carried: true }; }", "      return { ok: false, reason: 'NOT_IN_TAPE_UNIVERSE' };", "      return { ok: true, requested: true, depth: r.depth };", "      writeCurrentFeatureSnapshot(p.coin, snapshot, { tsMs, session: sessionDate(new Date(tsMs)), symbol: p.symbol });", "      writeEvent('BOOK_SNAPSHOT_REQUESTED', { symbol, depth: r.depth, reason: String(reason ?? '').slice(0, 120) });", "      writeSnapshot(snapshot);", "    // the venue answers a subscription with exactly one book snapshot: a symbol admitted after ours needs a real one", "    bookSnapshotAsked.delete(symbol);", "    const pinned = pinnedSymbols();", "    depthOf: (symbol) => pairs.get(symbol)?.depth ?? null,", "    ensureSubscribed(symbol, entry) {", "    feedGuard(() => executionFeed.ingest(raw, lastAnyMsgMs));", "    requestBookSnapshot(symbol, reason = null) {", "    { method: 'subscribe', params: { channel: 'book', symbol: [symbol], depth: pair.depth, snapshot: true } },", "    { method: 'unsubscribe', params: { channel: 'book', symbol: [symbol], depth: pair.depth } },", "    },", "    },", "  // bounded admission seam: the feed may ask for a symbol the tape already carries (or re-adopt a HELD symbol from its", "  // recorded instrument identity); it cannot widen the venue, raise depth or lift resource caps", "  ] };", "  const bookSnapshotAsked = new Map(); // symbol -> clock of the last execution-feed book-snapshot request", "  const feedGuard = (fn) => { if (!executionFeed) return; try { fn(); } catch (err) { feedErrors += 1; if (feedErrors <= 3) log(`execution feed error #${feedErrors} (ignored): ${err?.message ?? err}`); } };", "  const observe = (fn) => { if (!observer) return; try { fn(); } catch (err) { observerErrors += 1; if (observerErrors <= 3) log(`tape observer error #${observerErrors} (ignored): ${err?.message ?? err}`); } };", "  const pinnedSymbols = () => { try { return executionFeed ? executionFeed.pinned() : new Set(); } catch { return new Set(); } };", "  feedGuard(() => executionFeed.bindTape({", "  if (!pair) return { ok: false, reason: 'NOT_IN_TAPE_UNIVERSE' };", "  if (!socketOpen) return { ok: false, reason: 'SOCKET_NOT_OPEN' };", "  if (lastRequestMs !== null && nowMs - lastRequestMs < minIntervalMs) return { ok: false, reason: 'RATE_LIMITED', retryAfterMs: minIntervalMs - (nowMs - lastRequestMs) };", "  if (unavailable) return { ok: false, reason: 'PAIR_UNAVAILABLE' };", "  let observerErrors = 0; let feedErrors = 0;", "  return { ok: true, requested: true, depth: pair.depth, messages: [", "  writeCurrentFeatureSnapshot,", "  }));", "// EXECUTION-1 seam (ticket §4.2-4.3): an OPTIONAL execution feed receives the raw accepted socket text (exact lexemes) plus", "// MARKET-LAB seam: an OPTIONAL observer receives copies of ACCEPTED trades and applied books (values only, plus the", "// connect / disconnect epochs, may pin held / pending symbols (never shed, never dropped at the session refresh) and may ask", "// declared exception: requestBookSnapshot, where the feed asks for the venue's book snapshot for a symbol it admitted AFTER", "// documented resynchronisation path: the tape's own book desynchronises for the round trip and is re-established by the same", "// for a bounded on-demand subscription of a symbol the tape already carries. It never mutates the tape's own books — with one", "// logged (bounded), never propagated — the tape's health, books, snapshots and features do not depend on it.", "// pure: what a book-snapshot request may do right now, and the exact subscription messages it would send", "// receipt clock). It is called after the tape has already written its own truth; an observer exception is counted and", "// the tape's subscription (the venue sends one snapshot per subscription, so a later admission has none). That is the", "// venue snapshot. It never widens the venue, never raises depth and is rate-limited per symbol.", "export async function runTape({ minutes = null, chaosAfterSec = null, log = console.log, executionFeed = null, observer = null } = {}) {", "export const BOOK_SNAPSHOT_MIN_INTERVAL_MS = 15_000;", "export function bookSnapshotRequest({ symbol, pair, unavailable = false, socketOpen, lastRequestMs = null, nowMs, minIntervalMs = BOOK_SNAPSHOT_MIN_INTERVAL_MS }) {", "}"], removed: ["        .filter((p) => !p.major && !unavailable.has(p.symbol))", "      if (!incoming.has(symbol)) removed.push(symbol);", "      writeSnapshot({ ts: nowIso(), coin: p.coin, tapeState, ...bf, ...flows.get(p.symbol).features() });", "export async function runTape({ minutes = null, chaosAfterSec = null, log = console.log } = {}) {"] },
    // MARKET / SOCRATES CLOSEOUT (R01-R07): the doctrine ALSO gains one appended closeout appendix (the seven repaired laws) — authorized below in the same exact-line delta
    // MARKET / SOCRATES REMAINING CLOSEOUT (revision 2, P1-P5): ONE more appended appendix (the five remaining laws) — its exact lines are added to the same delta by REV2_DOCTRINE_LINES below
    // MARKET-LAB / SOCRATES v2: the doctrine gains ONE appended section describing the v2 machinery; the permanent sentence and every existing line stay (exact added-line delta, nothing removed)
    'doctrine/SOCRATES.md': { added: ["", "", "", "## v2 — the research interpreter over real market evidence (MARKET LAB)", "The sentence above still governs. v2 adds the machinery, not authority:", "- **Evidence v2** (`serpent-evidence-2`, `evidence/contract-v2.js`, `evidence/research-builder.js`) is a compact", "  dossier of family summaries derived from a sealed market context plus the read-only Social projection. v1 packets,", "  ids, validators and dispatch are untouched; v2 rejects unknown versions and keys and never converts old artifacts.", "- **Analysis v2** (`socrates-analysis-2`, `socrates/contract-v2.js`) is assembled by the host from the model's raw", "  text: ids, packet binding, schema version and calibration labels are host-derived; a model-supplied id is refused.", "  Every citation must resolve inside the packet; a thesis without support is invalid; execution vocabulary fails.", "- **The case runtime** (`socrates/runtime.js`) is a state machine: packet validity gate, budget reservation persisted", "  before dispatch, the production client, at most two accepted attempts and one follow-up round through the bounded", "  broker, a new packet at Q1 >= result receipt, the original packet preserved, sealed CASE bundles that verify offline.", "  Terminal states are honest: COMPLETED, INITIAL_REPORT_ONLY, MODEL_FAILED, BUDGET_BLOCKED, DEADLINE_EXCEEDED,", "  PACKET_INVALID, CANCELLED. A recorded response is labelled RECORDED_RESPONSE; a reused answer discloses its age.", "- **Spend** is a law, not a hope: zero caps mean zero calls, the reservation precedes the request, an ambiguous", "  timeout stays counted as spent, a restart cannot reset spent quota, and no cheaper model is substituted silently.", "- **Hostile text** (headlines, excerpts, provider errors, broker diagnostics) reaches the model only as data inside the", "  dossier; the only HTTP host is the model host; provider URLs are chosen by code, never by the model or by a source.", "- **The corpus** (`socrates/corpus.js`, twelve cases, development and held-out split) and the rubric", "  (`socrates/evaluate.js`) measure citation validity, unsupported claims, temporal errors, conflated measurements,", "  request selection and honest refusal. Scores are an implementation assessment, never stage calibration, a backtest", "  or profit proof. A model may reasonably choose a different supported interpretation.", "- Authority remains NONE. Research output is read by a person through sealed files and the read-only cockpit drawer.", "  No trade button, no order suggestion, no paid toggle, no import of orders, ledger, cost, risk, Judge or Watch.", "", "", "", "", "## Closeout appendix — the seven laws of the market / Socrates repair (R01–R07)", "1. **One accounting authority stands before every dispatch.** A provider request is reserved in the stable research", "   journal (`<research-root>/accounting`) at the dispatch boundary, in native charge units, under day / month / smoke /", "   entitlement / metered caps, or it never reaches the wire. Probe, catalog, acquisition, broker and shared requests use", "   the same guard; refusal is truthful usage; an ambiguous failure stays counted; a restart, a rollover, a repeated", "   attestation or a clock rollback never re-grant. The model reservation covers the worst billed input class actually", "   enabled and needs an exact provider token count.", "2. **Completeness is a positive fact.** A trade window is COMPLETE only over a recorded subscription-continuity fact;", "   a quiet proven interval is a valid zero; anything else is PARTIAL or UNKNOWN_COVERAGE with observed rows labelled", "   observed. An options surface is complete only against a recorded instrument census; a summary and its ticker are one", "   contract; a ratio over a subset says so.", "3. **Every registered metric has one explicit mapping** from acquisition to context component to packet item, bounded", "   and reconciled; absent inputs are MISSING items with no value, never known items with null fields.", "4. **The broker satisfies the question that was asked**: per metric, per subject scope, on source and knowledge clocks,", "   with every constituent of a derived metric present and compatible; a cache hit is a fresh binding; the same question", "   under a new key is never bought twice.", "5. **Recording is bounded and every case cites an immutable prefix**: segments rotate before their bound, retention is", "   bounded with counted eviction facts, a recording failure stops admission with its first error preserved, and a case's", "   inputs resolve offline through sealed segments whose identity is bound to their bytes.", "6. **Close is an ownership barrier**: admission stops, owned requests are aborted, the drain is bounded, open", "   reservations are preserved as unresolved spend, the lock is released once, and no late result becomes a report.", "7. **There is one schema truth**: every component value and support obeys its metric's closed schema at the builder,", "   the candidate, the reader and the verifier; a legacy context is rejected, never converted; a resealed mutation is", "   exposed by recomputation from the source when the source is supplied, and labelled as unverified when it is not.", "Authority remains NONE. Readiness is BLOCKED until the owner supplies entitled keys, attested plans and a dollar", "ceiling; no independent acceptance seal is issued by the delivering session.", ...REV2_DOCTRINE_LINES, ...REV3_DOCTRINE_LINES], removed: [] },
  ...
  const REPINNED = {
    ...
    // LEAN PASS 4a (social-official cut): the census dropped the Meta / TikTok / YouTube / official-StockTwits providers, and
    // rumor2/social.js dropped the retired-provider mentions from its SOCIAL_FINANCE comment and retention firewall. Both pin
    // their complete resulting bytes (the exclusion doctrine lives in attic/doctrine/SOCIAL.md). Tests: test/social-census.test.js,
    // test/rumor2-authority.test.js (R2A-SOCIAL-6/7). rumor2/social-foundation.js dropped its retired-module comment reference.
    'rumor2/social-registry.js': '5272e921a82bb314f515004fe6d41f8eae0da99fd80be306b9f1c582663c2d82',
    'rumor2/social.js': '583d589323cb9f0e88f5749fabc1fb3a135460dc9a664e926afc56fcb01aa4e7',
    'rumor2/social-foundation.js': '512ec2b27b1ffcee7799837554ecd97769704daedfe726de74b335781f9bd1ba',
  ...
  for (const f of pinned) {
    ...
    const authorizedAdded = f === 'rumor2/social.js'
      ? [...AUTHORIZED_DELTA[f].added.filter((line) => !line.includes('shingleIndex.set(sh, new Map())')), "    if (na.length > 0) { if (!familyByExactText.has(na)) familyByExactText.set(na, fam); const fi = families.length - 1; for (const sh of fam.shingles) { let list = shingleIndex.get(sh); if (!list) { list = []; shingleIndex.set(sh, list); } list.push(fi); } }"]
      : AUTHORIZED_DELTA[f].added;
    assert.deepEqual(added.sort(), [...authorizedAdded].sort(), `${f}: only the SOCIAL-5 authorized lines were added`);
    assert.deepEqual(removed.sort(), [...AUTHORIZED_DELTA[f].removed].sort(), `${f}: only the SOCIAL-5 authorized lines were replaced`);
  }
});

// Keep current authority boundaries independently executable when an older
// historical byte-differential reports unrelated intentional source changes.
// The historical assertions above remain intact and still report those changes.
test('SCOPE-9. current catalog consumers and Social authority fences remain exact', () => {
