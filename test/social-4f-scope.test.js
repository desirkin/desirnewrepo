...
test('SCOPE-8 (9/10). protected surfaces are byte-identical to 9c17372; authority: the 4F modules import only inside their tier and touch no trading, control, or model surface', () => {
  ...
  const AUTHORIZED_DELTA = {
    ...
    'tape/run.js': { added: ["            checksumVerified = check.ok;", "            levels: () => ({ bids: [...book.bids.entries()].sort((a, b) => b[0] - a[0]).slice(0, 100).map(([price, qty]) => [price, qty]), asks: [...book.asks.entries()].sort((a, b) => a[0] - b[0]).slice(0, 100).map(([price, qty]) => [price, qty]) }) }));", "          let checksumVerified = null;", "          observe(() => observer.onBook({ coin: coinFromSymbol(d.symbol), symbol: d.symbol, receivedTs: Date.now(), synced: book.synced, checksumVerified, pricePrecision: book.pricePrecision, qtyPrecision: book.qtyPrecision,", "          observe(() => observer.onTrade({ coin: coinFromSymbol(t.symbol), symbol: t.symbol, side: t.side, qty: t.qty, price: t.price, eventTs: Date.parse(t.timestamp), receivedTs: Date.now(), tradeId: t.trade_id ?? null, ordType: t.ord_type ?? null, snapshot: msg.type === 'snapshot' }));", "      // ONE captured owner clock per snapshot: the appended record, and the passive current", "      // feature file (SOCIAL-5 \u00a736.7 read-only bridge) carry the SAME computed object and instant", "      const snapshot = { ts: new Date(tsMs).toISOString(), coin: p.coin, tapeState, ...bf, ...flows.get(p.symbol).features(tsMs) };", "      const tsMs = Date.now();", "      writeCurrentFeatureSnapshot(p.coin, snapshot, { tsMs, session: sessionDate(new Date(tsMs)), symbol: p.symbol });", "      writeSnapshot(snapshot);", "  const observe = (fn) => { if (!observer) return; try { fn(); } catch (err) { observerErrors += 1; if (observerErrors <= 3) log(`tape observer error #${observerErrors} (ignored): ${err?.message ?? err}`); } };", "  let observerErrors = 0;", "  writeCurrentFeatureSnapshot,", "// MARKET-LAB seam: an OPTIONAL observer receives copies of ACCEPTED trades and applied books (values only, plus the", "// logged (bounded), never propagated \u2014 the tape's health, books, snapshots and features do not depend on it.", "// receipt clock). It is called after the tape has already written its own truth; an observer exception is counted and", "export async function runTape({ minutes = null, chaosAfterSec = null, log = console.log, observer = null } = {}) {"], removed: ["      writeSnapshot({ ts: nowIso(), coin: p.coin, tapeState, ...bf, ...flows.get(p.symbol).features() });", "export async function runTape({ minutes = null, chaosAfterSec = null, log = console.log } = {}) {"] },
  ...
  assert.deepEqual(consumers, ['market-lab/providers/kraken-spot.js', 'survey/wideeye.js']); // MARKET-LAB: the Kraken client reuses the pure pair normalizer
