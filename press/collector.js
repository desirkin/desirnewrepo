// PRESS — the bounded publisher-observation collector (owner scope P05-P08). DARK by default: nothing runs unless the paper
...
// checkpoint advances (a checkpoint never runs ahead of its records). Failure classes stay distinct in the status file: NOT_MODIFIED, EMPTY_FEED (access
...
import { itemToObservation, pressObservationError, PRESS_LIMITS } from './parse.js';
...
export const PRESS_STATES = Object.freeze(['DISABLED', 'IDLE', 'OBSERVED', 'NOT_MODIFIED', 'EMPTY_FEED', 'PARSE_FAILED', 'RATE_LIMITED', 'FAILED', 'RETENTION_CAP']);
...
export function startPress({ env = process.env, dataDir = defaultDataDir(), fetchImpl = fetch, clock = () => Date.now(), log = console.log, sources = PRESS_SOURCES, timers = { setTimeout, clearTimeout, setInterval, clearInterval }, firstDelayMs = PRESS_LIMITS_RUNTIME.firstDelayMs, signals = true } = {}) {
  ...
  async function poll(id) {
    ...
    try {
      ...
      let admitted = 0; let skipped = 0;
      for (const item of parsed.items.slice(0, PRESS_LIMITS.maxItemsPerPoll)) {
        st.counters.observed += 1; const m = itemToObservation(item, { source: s, receiptTs, feedKind: parsed.kind }); if (m.skip) { st.counters.skipped += 1; skipped += 1; continue; }
    ...
    let i = 0; for (const [id, st] of S) { if (!st.selected) continue; const first = timers.setTimeout(() => { timerSet.delete(first); poll(id).catch(() => {}); }, firstDelayMs + i * PRESS_LIMITS_RUNTIME.staggerMs); timerSet.add(first); const iv = timers.setInterval(() => { poll(id).catch(() => {}); }, st.source.cadenceSec * 1000); timerSet.add(iv); i += 1; }
    writeStatus();
  ...
  log(`[press] watching ${[...S.values()].filter((s) => s.selected).length} publisher feed(s) — headline / link observations only, authority NONE`);
  return handle;
