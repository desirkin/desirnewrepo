...
export function readPressStatus(dataDir) { try { const f = pressStatusFile(dataDir); if (!existsSync(f)) return null; const s = readJsonBounded(f); return s && s.v === PRESS_STATUS_VERSION && s.sources && typeof s.sources === 'object' ? s : null; } catch { return null; } }
...
export function readPressObservations(dataDir, { sourceId = null, sinceReceiptTs = null, limit = 100 } = {}) {
  ...
  const bounded = readJsonlTail(f, { maxBytes: MAX_READ_BYTES }); out.truncatedRead = bounded.truncated; if (bounded.torn) out.corrupt += 1; const lines = bounded.lines; const kept = [];
