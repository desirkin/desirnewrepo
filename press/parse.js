// PRESS — pure mapping from a parsed feed item to ONE publisher observation (`press-observation-1`). Clocks: `publishedTs`
// is the feed's own publication clock (null when the feed gives none — never fabricated from receipt); `receiptTs` is when
// the bytes arrived; `knownAtTs` = receiptTs (what the app knew, when). Identity: sha256(sourceId | guid-or-link) — the same
// article through two publishers is two observations with two identities and is NEVER merged into corroboration here.
// Content is untrusted text: bounded, tags stripped by the shared parser, links accepted only as https URLs whose host is
// recorded; bodies are never fetched. (LEAN PASS 4a retired the aggregator per-item <source> split; every source is a direct
// RSS publisher, so publisher/publisherHost are the source's own name/host.)
...
export function itemToObservation(item, { source, receiptTs, feedKind }) {
  ...
  return { observation: Object.freeze({
    ...
    publisher: source.name, publisherHost: source.host,
...
export const PRESS_OBSERVATION_KEYS = Object.freeze(['v', 'observationId', 'itemKey', 'sourceId', 'sourceKind', 'publisher', 'publisherHost', 'feedKind', 'title', 'summary', 'link', 'linkHost', 'publishedTs', 'receiptTs', 'knownAtTs', 'coverage', 'authority', 'bodyFetched']);
export function pressObservationError(o) {
  ...
  if (!/^[0-9a-f]{64}$/.test(o.observationId ?? '') || o.observationId !== sha256(`${o.sourceId}|${o.itemKey}`)) return 'observationId is not the derived identity'; if (typeof o.sourceId !== 'string' || o.sourceKind !== 'PUBLISHER') return 'source';
  ...
  if (!(o.publisher === null || typeof o.publisher === 'string') || !(o.publisherHost === null || typeof o.publisherHost === 'string')) return 'publisher';
  return null;
}
