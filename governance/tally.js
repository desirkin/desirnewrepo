...
export async function tallyGql(query, variables, { fetchImpl = fetch, timeoutMs = 15_000, env = process.env, signal = null } = {}) {
  ...
  const response = await fetchJsonBounded(TALLY_API, {
    host: 'api.tally.xyz',
    method: 'POST',
    fetchImpl,
    timeoutMs,
    ...
    jsonReviver: exactIntegers,
  });
  if (response.outcome === 'RATE_LIMITED') {
    throw new TallyRetry429(Math.min(86400, response.retryAfterSec ?? 900));
  }
  if (response.outcome !== 'OK') throw new Error(`tally ${response.reason ?? response.outcome}`);
...
export function normalizeTallyProposal(raw, governorId) {
  ...
  if (!TALLY_GOVERNOR_RE.test(governorId) && raw && typeof raw === 'object' &&
      typeof raw.status === 'string' && raw.status === raw.status.toUpperCase()) {
    if (raw.governor?.id != null && raw.governor.id !== governorId) return null;
