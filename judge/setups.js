...
export function freezeReferences({ setupId, ind, spec, fast, event = null, triggerTs, strategyVersion = STRATEGY_VERSION }) {
  ...
  if (setupId === 'CATALYST_TRANSMISSION') { if (!preEventSnapshotBound(event)) return null; const p0 = event.p0; const trigger = M.add(p0, M.mul('0.1', atr)); return { ...base, event: { eventId: event.eventId, knownAtTs: event.knownAtTs, taxonomy: event.taxonomy, caseId: event.caseId, analysisId: event.analysisId, p0Evidence: event.p0Evidence }, p0, preEventH20: d(ind.h20), preEventL20: d(ind.l20), preEventLow5: d(ind.low5), triggerLevel: trigger, structuralStop: M.sub(M.min(p0, d(ind.low5)), buffer), scenarioRaw: M.add(p0, M.sub(d(ind.h20), d(ind.l20))), maxEntryLevel: M.add(p0, M.mul('1', atr)) }; }
