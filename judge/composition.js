...
import { ledgerView } from '../execution/ledger-view.js';
...
export async function composeJudge({ policyFile, mode, accountId = null, env = process.env, log = console.log, db = null, journal = null, clock = null, feed = null, transport = null, WebSocketImpl = null, specs = null, history = null, caseSource = null, casesDir = null, controlsSource = null, nominations = null, recordDir = null, allowPrivate = () => false, allowOrders = () => false, requireDb = null, writeProjection = true, codeDigest = undefined, caseWorker = null, exchangeContext = 'kraken-spot', experimentId = null, armRule = null, exitPolicy = null, verdictSink = null, learningActivationSource = null, dynamicSizing = null, persistenceHealth = null, permissionIncreaseAllowed = null, paperLatencySource = null, writerLockWait = null }) {
  ...
  if (!jr) { if (needDb) { ownDb = db ?? new Db({ log }); if (!ownDb.configured()) throw new JournalError('DB_REQUIRED', `${mode} needs DATABASE_URL: the journal authority is PostgreSQL`); if (!(await ownDb.connect())) throw new JournalError('DB_UNAVAILABLE', 'database unreachable: no account authority, no dispatch'); await runMigrations(ownDb, { log }); jr = createPgJournal({ db: ownDb, log, lockWait: writerLockWait }); } else jr = createMemoryJournal({ log }); }
  ...
  const dailyLockView = () => { try { const s = dispatcher.state(); const sess = s?.performance?.session ?? null; const opening = sess?.openingEquity ?? null; const supported = Boolean(sess && lockThresholds && M.isPositive(opening ?? '0')); const pnlPct = supported ? M.toStatistic(M.div(M.mul(sess.dayPnl, '100'), opening, 6, 'HALF_UP')) : null; return { sessionDate: sess?.date ?? null, openingEquityUsd: opening === null ? null : Number(opening), dayPnlUsd: sess ? Number(sess.dayPnl) : null, pnlPct, level: supported ? lockLevelForPnlPct(pnlPct, lockThresholds) : 'NONE', thresholds: lockThresholds ?? null, supported }; } catch { return { sessionDate: null, openingEquityUsd: null, dayPnlUsd: null, pnlPct: null, level: 'NONE', thresholds: lockThresholds ?? null, supported: false }; } };
  ...
  function projection() {
    ...
    return { projectionVersion: PROJECTION_VERSION, compositionVersion: COMPOSITION_VERSION, ts: nowTs(), accountId: acct, mode: s?.mode ?? mode, runMode: mode, accountKind: kind, policyName: policy.policyName, policyDigest, codeDigest: treeDigest, revision: dispatcher.revision(), writerLost: dispatcher.writerLost(), adapter: adapter.kind, credentialsPresent: Boolean(credentials), keyFingerprint: fp, authorization,
      ...
      // ONE bankroll, ONE daily-lock law (lean trim step 1, 2026-09-14): the journal's session P&L against its opening equity
      // under config.locks — the same lockLevel() the Judge consults — published for the posture machine, the paper preflight /
      // readiness snapshot and the cockpit. The legacy JSONL ledger no longer computes a second answer.
      dailyLock: dailyLockView(), ledger: ledgerView(s, { nowMs: nowTs() }),
