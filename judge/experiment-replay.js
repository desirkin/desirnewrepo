...
import { evaluateAccount, isExecutedClosedPosition, seededControlSelected } from './challengers.js';
...
// Ticket 4 (2026-09-15): the D4 experiment is FLIPPED. The live exit law is now no-target, so the REFERENCE arms
...
function createMatchedRunner({ ref, policy, policyDigest, codeDigest, clock, controls, exitPolicy, idPrefix, log }) {
  ...
  return { feed, mirror,
    ...
    report(now) { const refState = ref.dispatcher.state(); const rows = []; for (const [positionId, ep] of episodes) { const rp = refState.positions[positionId]; const ap = ep.dispatcher.state().positions[positionId]; const altMarks = ep.watch.marks(now); const am = altMarks.marks.find((m) => m.positionId === positionId) ?? null; const side = (p, mark) => (p ? { state: p.state, realizedPnl: isExecutedClosedPosition(p) ? p.realizedPnl : null, exitReason: p.exit?.primaryReason ?? null, censored: p.state !== 'FLAT' ? censoredPosition(p, mark) : null } : null); const paired = isExecutedClosedPosition(rp) && isExecutedClosedPosition(ap); rows.push({ positionId, decisionId: ep.decisionId, accountId: ep.accountId, clonedEvents: ep.clonedEvents, clonedTs: ep.clonedTs, ref: side(rp, null), alternative: side(ap, am), paired, difference: paired ? M.sub(ap.realizedPnl, rp.realizedPnl) : null }); }
      return { arm: 'D4_PLANNED_TARGET_MATCHED', kind: 'MATCHED', outcome: rows.length ? 'SCORED' : 'UNSCORABLE', reason: rows.length ? null : 'NO_REF_COMBINED_ENTRY_REACHED_FINAL_R', follows: ref.accountId, episodes: rows, paired: rows.filter((r) => r.paired).length, censored: rows.filter((r) => !r.paired).length, refused, law: 'per-episode isolated accounts with the identical confirmed entry and ONLY the planned full exit ENABLED against the live no-target reference; never summed into a funded USD 500 return' }; },
  };
}
