...
import { readBundle, verifyBundle, createBundleStores, TIE_ORDER_VERSION, BUNDLE_VERSION, CAPABILITY_REQUIREMENTS } from './experiment-bundle.js';
...
// ---- the engine --------------------------------------------------------------------------------------------------------------------
export async function replayExperiment({ bundleDir, policyFile, arms = null, experimentId, seed = null, codeDigest, log = () => {}, stopAtSeq = null, expectedBinding = null, tickMs = HEARTBEAT_MS, bucketMs = BUCKET_MS }) {
  ...
  const caps = verified.capabilities; const sourcePrefix = verified.binding?.sourcePrefix ?? null;
  ...
  // ---- the stream ----
  const timers = { bucket: null, heartbeat: null }; let lastSeq = 0; let lastTs = null; let clockBackwards = 0; let records = 0; const counts = {}; let stoppedAtSeq = null; let fatal = null;
  ...
  try {
    ...
    while (!cur.done) { const rec = cur.value; const nxt = it.next(); records += 1; counts[rec.kind] = (counts[rec.kind] ?? 0) + 1;
  ...
  return { ok: !fatal, outcome: fatal ? 'INCOMPLETE' : stoppedAtSeq !== null ? 'STOPPED_AT_SEQ' : 'COMPLETE', engine: REPLAY_ENGINE_VERSION, codeIdentity: codeDigest ? 'KNOWN' : 'UNKNOWN_DIRTY_TREE', bundleVersion: BUNDLE_VERSION, tieOrderVersion: TIE_ORDER_VERSION, tieOrderLaw: TIE_ORDER_LAW, experimentId, seed: useSeed, policyDigest, codeDigest, strategyVersion: policy.strategyVersion, sourcePrefix, bundle: { dir: bundleDir, records, counts, firstTs: first, lastTs, lastSeq, clockBackwards, capabilities: caps.evaluations }, arms: report.arms, observation, stoppedAtSeq, fatal, law: 'independent REPLAY arms over one sealed input stream; open positions are CENSORED with factual marks or an unknown mark; no PAPER / LIVE account, provider, model or exchange is touched' };
}
