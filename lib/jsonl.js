...
const sleepSync = (milliseconds) => Atomics.wait(retryWait, 0, 0, milliseconds);
const ATOMIC_IO = Object.freeze({ mkdirSync, openSync, fsyncSync, closeSync, writeFileSync, renameSync, unlinkSync, sleepSync });

// Windows can transiently deny replacement while another process is replacing
// the same destination. Retry only that observed EPERM boundary, with the SAME
// complete temp file. Never unlink/truncate the target to force success. Seven
// bounded waits total 63 ms; persistent permission failures still surface.
// This preserves atomic last-writer-wins replacement, not read/modify/write
// serialization (the owning store must still provide its own writer lock).
function renameAtomicTemp(tmp, file, { io, platform }) {
  for (let attempt = 0; ; attempt++) {
    try { io.renameSync(tmp, file); return; }
    catch (error) {
      if (platform !== 'win32' || error?.code !== 'EPERM' || attempt >= 7) throw error;
      io.sleepSync(Math.min(2 ** attempt, 16));
    }
  }
}
...
function atomicWriteJsonImpl(file, obj, { pretty, sync, io, platform }) {
  ...
  try {
    ...
    renameAtomicTemp(tmp, file, { io, platform });
