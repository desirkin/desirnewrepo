// lib/serpent-runtime.js — the DATA-ONLY runtime spine (runtime unification step 1, 2026-09-14).
// Moved verbatim from tools/data-only-runtime.mjs: single-instance lock, runtime-status.json, persistence + external-quota
// restore, the governor, market catalogs, wide eye, broad Kraken, infra, video, discovery, gateway, RUMOR-2, ordered
// shutdown. The entry file keeps ONLY the safety env pins and calls startDataOnlyRuntime(). No behavior change; the next
// steps (docs/serpent/RUNTIME-UNIFICATION.md) fold the PAPER root onto this spine with a mode.
// This module still imports no trading composition: fly.js, paper/, judge/, execution/, watch/, tape/run.js, bin/cobra.js
// are absent from its import graph (test/data-only-runtime.test.mjs).
import path from 'node:path';
import { existsSync, mkdirSync, openSync, closeSync, readFileSync, unlinkSync, writeFileSync } from 'node:fs';
import { loadConfig, dataDir as resolveDataDir } from './config.js';
...
import { startPersistence, getPersistence } from '../persistence/runtime.js';
...
import { onceAsync, dataOnlySpending } from './data-only-lifecycle.js';

export async function startDataOnlyRuntime() {
