...
export async function restoreExternalQuota({ root, env = process.env, log, blockers, phase, starters = DEFAULT_QUOTA_STARTERS }) {
  ...
  try {
    ...
    // PUBLISH-FIX-3: never let the boot swallow this — c275d46b printed NO persistence lines at all, so whatever
    // startPersistence threw here was invisible. Log the error code/class BEFORE setting the blockers, always.
    const code = error?.code ?? error?.constructor?.name ?? 'UNKNOWN';
    log(`PERSISTENCE restore failed: ${code}`);
