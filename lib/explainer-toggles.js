...
// SOCRATES runtime activation (David's SOCRATES-toggle decision, 2026-09-15). The cockpit SOCRATES toggle is the
// day-to-day on/off for paid Socrates cases — no publish, no secret edit. This is the ONE place paid model calls turn
// on from the cockpit, and it is the intended one; the toggle endpoint is password-protected, which is the control.
//   OFF                                    -> { active:false, reason:'TOGGLE_OFF' }  (default-OFF file default; nothing spends)
//   ON but no ANTHROPIC_API_KEY            -> { active:false, reason:'CREDENTIAL_MISSING' }
//   ON, key present, env daily cap <= 0    -> { active:false, reason:'SOCRATES_CAP_ZERO' }
//   ON, key present, env daily cap > 0     -> { active:true }  (the caller caps the daily spend at min(config, env))
// Fail-closed: any read failure or absent toggle file resolves to OFF. `envDailyUsd` is always returned so the caller
// can compute the effective daily cap = min(config maxEstimatedUsdPerDay, envDailyUsd).
export function resolveSocratesActivation({ dataDir, env = process.env } = {}) {
  let socratesOn = false;
  try { socratesOn = readExplainerToggles({ dataDir }).socrates === true; } catch { socratesOn = false; }
  const keyPresent = typeof env[EXPLAINER_CAP_ENV.key] === 'string' && env[EXPLAINER_CAP_ENV.key].length > 0;
  const envDailyUsd = resolveDailyCap(env, EXPLAINER_CAP_ENV.socratesDailyUsd);
  if (!socratesOn) return { active: false, reason: 'TOGGLE_OFF', envDailyUsd };
  if (!keyPresent) return { active: false, reason: 'CREDENTIAL_MISSING', envDailyUsd };
  if (!(envDailyUsd > 0)) return { active: false, reason: 'SOCRATES_CAP_ZERO', envDailyUsd };
  return { active: true, reason: null, envDailyUsd };
}
