...
export const DEFAULT_STARTERS = Object.freeze({ startDataOnlyMarket, startWideEye, startBroadKraken, startVideo, startPublicDiscovery, startGateway, startRumor2 });
...
export async function startSharedCollectors({ root, config = collectorConfig(), env = process.env, governor, checkpoints, blockers, handles, log, phase, starters = DEFAULT_STARTERS }) {
  const { startDataOnlyMarket, startWideEye, startBroadKraken, startVideo, startPublicDiscovery, startGateway, startRumor2 } = starters;
  let wideEye = null;
  let rumor2 = null;
  let video = null;
  let market = null;
...
// Kraken capture and public discovery. Everything else in PAPER — wide eye, gateway, video, RUMOR-2 — is
