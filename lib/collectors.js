...
export const DEFAULT_STARTERS = Object.freeze({ startDataOnlyMarket, startWideEye, startBroadKraken, startPublicDiscovery, startGateway, startRumor2 });
...
export async function startSharedCollectors({ root, config = collectorConfig(), env = process.env, governor, checkpoints, blockers, handles, log, phase, starters = DEFAULT_STARTERS }) {
  const { startDataOnlyMarket, startWideEye, startBroadKraken, startPublicDiscovery, startGateway, startRumor2 } = starters;
  let wideEye = null;
  let rumor2 = null;
  let market = null;
  ...
  return { wideEye, rumor2, market, broadMarket, broadDayArchive, publicDiscovery, catalogSource, catalog };
}
...
// Kraken capture and public discovery. Everything else in PAPER — wide eye, gateway, RUMOR-2 — is
