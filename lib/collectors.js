...
export async function startSharedCollectors({ root, config = collectorConfig(), env = process.env, governor, checkpoints, blockers, handles, log, phase, starters = DEFAULT_STARTERS }) {
  ...
  // PUBLISH-FIX-4: one boot line names the process that owns this collector set,
  // so the contended writers (broad Kraken) have exactly one recorded owner.
  log(`COLLECTOR OWNERSHIP: pid ${process.pid} owns the DATA_ONLY collector set (market catalogs, wide eye, broad Kraken writer, gateway, RUMOR-2)`);
  let wideEye = null;
  let rumor2 = null;
  let market = null;
  ...
  if (!blockers.RUMOR2 && catalog) {
    try {
      ...
      rumor2 = startRumor2({
        ...
        journal: rumor2JournalStore({ lockWait: { waitMs: 180_000, intervalMs: 5_000 } }), // PUBLISH-FIX-5: boot path waits out a Republish overlap for the RUMOR-2 writer lock
...
export async function startPaperCollectorAdditions({ root, env = process.env, governor, checkpoints, blockers, handles, log, phase, catalogAccessor, starters = DEFAULT_STARTERS }) {
  ...
  // PUBLISH-FIX-4: name the process that owns the PAPER collector additions (the
  // broad Kraken writer among them) so the boot log records exactly one owner.
  log(`COLLECTOR OWNERSHIP: pid ${process.pid} owns the PAPER collector additions (market catalogs, broad Kraken writer)`);
  let market = null;
  let broadMarket = null;
