...
async function requestBounded(url, options, json) {
  ...
  const ctl = new AbortController(); let reader = null; let response = null; let timer; let failureStage = 'transport';
  ...
  try {
    for (let hop = 0; hop <= maxRedirects; hop++) {
      ...
      failureStage = 'response body read';
      for (;;) {
      ...
      failureStage = 'response UTF-8 decoding';
