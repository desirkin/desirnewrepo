...
function publicStoreGuard(guard) {
  if (!object(guard)) return null;
  return {
    status: diagnosticCode(guard.status) ? guard.status : 'UNAVAILABLE',
    coverage: diagnosticCode(guard.coverage) ? guard.coverage : 'UNKNOWN',
    checked: count(guard.checked), failed: count(guard.failed),
    permissionLock: typeof guard.permissionLock === 'boolean' ? guard.permissionLock : null,
    // No paths, payloads, provider values, raw error messages or SQL escape
    // this projection. Only closed/bounded diagnostic metadata is public.
    stores: (Array.isArray(guard.stores) ? guard.stores : []).slice(0, 64).map((store) => ({
      storeId: typeof store?.storeId === 'string' && /^[a-z0-9][a-z0-9:_-]{0,127}$/.test(store.storeId) ? store.storeId : 'invalid',
      status: diagnosticCode(store?.status) ? store.status : 'UNAVAILABLE',
      reason: diagnosticCode(store?.reason) ? store.reason : 'VERIFICATION_UNAVAILABLE',
    })),
  };
}
...
export function readDataOnlyRuntimeStatus({
  ...
  // runtime unification step 5: the canonical paths live under serpent/ (any mode); the data-only/ mirrors remain
  // readable for one release so a status written by a pre-step-5 process is still evaluated.
  const canonicalStatus = path.join(root, 'serpent', 'runtime-status.json');
  const statusFile = existsSync(canonicalStatus) ? canonicalStatus : path.join(root, 'data-only', 'runtime-status.json');
  const canonicalLock = path.join(root, 'serpent', 'runtime.lock');
  const lockFile = existsSync(canonicalLock) ? canonicalLock : path.join(root, 'data-only', 'runtime.lock');
...
export function dataOnlyPublicStatus(evaluated) {
  ...
  return {
    ...
    persistence: object(persistence) ? {
      ...
      permissionLock: typeof persistence.permissionLock === 'boolean' ? persistence.permissionLock : null,
      storeIntegrityLock: persistence.storeIntegrityLock === true,
      storeGuard: publicStoreGuard(persistence.storeGuard),
    } : null,
  };
}
...
export function dataOnlySensorSnapshot(evaluated, { now = Date.now() } = {}) {
  ...
  const persistenceHealthy = persistence?.restored === true && persistence?.status === 'HEALTHY' && persistence?.permissionLock !== true && persistence?.storeGuard?.permissionLock !== true;
  add('STORAGE', { id: 'PERSISTENCE', name: 'Durable PostgreSQL journal', state: persistenceHealthy ? 'ACTIVE' : persistence?.restored ? 'DEGRADED' : 'BLOCKED', lastSuccessTs: lastTs(persistence?.lastSuccessfulWriteTs ?? persistence?.lastSuccessfulReadTs), coverage: persistence?.databaseConfigured ? `configured; ${persistence.status ?? 'unavailable'}; store verification ${publicStatus.persistence?.storeGuard?.status ?? 'NOT_REPORTED'}` : 'DATABASE_URL not configured', blocker: persistenceHealthy ? null : persistence?.failureCategory ?? (persistence?.permissionLock ? 'PERSISTENCE_PERMISSION_LOCK' : 'NOT_VERIFIED_HEALTHY') });
