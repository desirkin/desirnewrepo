...
// ±`window` steps of clock skew. Returns true only on a match; ANY malformed
// input (bad secret, non-6-digit code, missing value) fails closed to false.
export function verifyTotp(secret, code, { now = Date.now, period = TOTP.periodSec, digits = TOTP.digits, algorithm = TOTP.algorithm, window = TOTP.window } = {}) {
  ...
  if (supplied === null || !new RegExp(`^\\d{${digits}}$`).test(supplied)) return false;
  const counter = Math.floor(now() / 1000 / period);
  for (let w = -window; w <= window; w++) {
    if (codeEqual(hotp(key, counter + w, { digits, algorithm }), supplied)) return true;
  }
  return false;
}
