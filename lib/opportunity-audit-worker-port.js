  ...
  const close = () => {
    ...
    closePromise = (async () => {
      ...
      const physicalTimeout = async () => {
        ...
        if (workerLive) return { stopped: false, physicalExit: false, reason: 'WORKER_EXIT_TIMEOUT' };
        stopped = true; return { stopped: true, physicalExit: true };
      };
      ...
      if (!workerLive) { await closedPromise; stopped = true; return { stopped: true, physicalExit: true }; }
