...
function createPort(options = {}, startWorker = workerFactory) {
  ...
  const settle = async ({ item, resolution, recordedTs } = {}) => {
    if (pendingItemError(item) || !safeTs(recordedTs) || !plain(resolution)) fail('REQUEST_INVALID');
    const payload = { item: clone(item), resolution: clone(resolution), recordedTs };
