...
// by QUEUED_NOT_COMMITTED.
import { createHash } from 'node:crypto';
import path from 'node:path';
import { Worker } from 'node:worker_threads';

export const OPPORTUNITY_AUDIT_WORKER_PORT_VERSION = 'opportunity-audit-worker-port-1';
export const OPPORTUNITY_AUDIT_WORKER_PROTOCOL = 'opportunity-audit-worker-1';
...
const FAILURE_CODES = new Set([
  ...
  'WORKER_ERROR', 'WORKER_EXITED', 'WORKER_RESOURCE_LIMIT_INVALID',
...
function createPort(options = {}, startWorker = workerFactory) {
  const optionKeys = new Set([
    ...
    'wideEyeComponent', 'requestTimeoutMs', 'batchTimeoutMs', 'closeTimeoutMs',
  ]);
  ...
  const {
    ...
    minFrameIntervalMs = 15 * 60 * 1000, wideEyeComponent = null,
  ...
  if (!enabled) {
    const disabled = Object.freeze({
      ...
      lastFailedBatch: null, failed: null, forcedTermination: false,
    ...
    return Object.freeze({
      ...
      status: () => disabled,
      close: async () => ({ stopped: true, physicalExit: true }),
    });
  }
  ...
  const workerConfig = Object.freeze({
    ...
    horizonsMs: [...horizonsMs], minFrameIntervalMs, wideEyeComponent: clone(wideEyeComponent),
  });
  ...
  let lastCommittedBatch = null; let lastFailedBatch = null; let failed = null;
  ...
  const ensureWorker = (allowClosing = false, allowFailed = false) => {
    ...
    worker.once('exit', (code) => {
      ...
      if (!closing && !stopped && failed === null) latch(code === 0 ? 'WORKER_EXITED' : 'WORKER_ERROR');
    });
  };

  const rpc = (operation, payload, timeoutMs = requestTimeoutMs, allowClosing = false, allowFailed = false) => {
  ...
  const close = () => {
    ...
    closing = true;
    closePromise = (async () => {
      if (!workerLive) { stopped = true; return { stopped: true, physicalExit: true }; }
      const requestAtClose = pendingRequest;
      if (requestAtClose) await requestAtClose.settled;
      if (failed && forcedTermination) {
        await worker.terminate().catch(() => {}); await closedPromise; stopped = true;
        return { stopped: true, physicalExit: true };
      }
      try { await rpc('CLOSE', {}, closeTimeoutMs, true, true); }
      catch {
        if (workerLive) await worker.terminate().catch(() => {});
      }
      await closedPromise; stopped = true;
      return { stopped: true, physicalExit: !workerLive };
    })();
    return closePromise;
  };
  ...
  const status = () => Object.freeze({
    ...
    lastDurableFrameId, lastCommittedBatch, lastFailedBatch,
    failed: failed === null ? null : { ...failed },
    forcedTermination,
    authority: 'NONE', trainingAuthority: 'NONE', runtimeWired: false,
  });

  return Object.freeze({ beforeSweep, afterSweep, status, close });
}
