...
import { noaaKpObservations, noaaScalesObservation, ripeRoutingStatusObservation, cloudflareBgpTimeseriesObservation, infraObservationError, RIPE_RESOURCE_RE } from './parse.js';
...
export const ripeResourcesFromEnv = (env = process.env, max = 16) => { const raw = String(env.INFRA_RIPE_RESOURCES ?? '').split(',').map((s) => s.trim()).filter(Boolean); const valid = raw.filter((r) => RIPE_RESOURCE_RE.test(r)); return { valid: [...new Set(valid)].slice(0, max), invalid: raw.filter((r) => !RIPE_RESOURCE_RE.test(r)), truncated: valid.length > max }; };
export const cloudflareScopeFromEnv = (env, s) => { const asnRaw = String(env[s.scopeEnv.asn] ?? '').split(',').map((x) => x.trim()).filter(Boolean); const asn = asnRaw.filter((x) => /^\d{1,10}$/.test(x)).slice(0, 8); const prefix = typeof env[s.scopeEnv.prefix] === 'string' && RIPE_RESOURCE_RE.test(env[s.scopeEnv.prefix]) && !/^AS/i.test(env[s.scopeEnv.prefix]) ? env[s.scopeEnv.prefix] : null; const dr = typeof env[s.scopeEnv.dateRange] === 'string' && /^(1d|2d|7d|14d|28d|12w|24w|52w)$/.test(env[s.scopeEnv.dateRange]) ? env[s.scopeEnv.dateRange] : s.defaults.dateRange; return { asn, prefix, dateRange: dr, aggInterval: s.defaults.aggInterval, invalidAsn: asnRaw.length - asn.length }; };
...
export function startInfra({ env = process.env, dataDir = defaultDataDir(), fetchImpl = fetch, clock = () => Date.now(), log = console.log, sources = INFRA_SOURCES, timers = { setTimeout, clearTimeout, setInterval, clearInterval }, firstDelayMs = INFRA_RUNTIME.firstDelayMs, signals = true } = {}) {
  ...
  const gateOf = (s) => { if (!selected.has(s.id)) return 'DISABLED'; if (s.credentialEnv && !(typeof env[s.credentialEnv] === 'string' && env[s.credentialEnv].length)) return 'CREDENTIAL_REQUIRED'; if (s.configEnv && ripeResourcesFromEnv(env, s.maxResources).valid.length === 0) return 'CONFIG_REQUIRED'; return 'IDLE'; };
  ...
  const admit = (st, o, file) => { if (st.seenSet.has(o.observationId)) { st.counters.duplicates += 1; return false; } const e = infraObservationError(o); if (e) { st.counters.rejected += 1; return false; } appendJsonl(file, o); st.counters.admitted += 1; st.seen.push(o.observationId); st.seenSet.add(o.observationId); while (st.seen.length > INFRA_RUNTIME.seenCap) st.seenSet.delete(st.seen.shift()); return true; };
