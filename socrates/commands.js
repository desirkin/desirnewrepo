...
export async function runCaseCommand({ packetDir, policy, env = {}, out, contextDir = null, captureDir = null, budgetDir = null, recordedResponseFile = null, reevaluation = false, fetchImpl = globalThis.fetch, clock = () => Date.now(), log = () => {}, limits = RESOURCE_DEFAULTS, owner = null }) {
  ...
  try {
    const handle = rt.runCase({ packet: pk.packet, marketContext: ctx?.context ?? null, out: real, reevaluation, contextMap: pk.contextMap });
...
export const runVerify = (caseDir, { limits = RESOURCE_DEFAULTS } = {}) => { const v = verifyCase(caseDir, { limits }); return deepFreeze({ ok: v.ok, command: 'verify', caseDir: path.resolve(caseDir), ...v }); };
