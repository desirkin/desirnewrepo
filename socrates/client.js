...
export function createAnthropicClient({ apiKey, apiHost = 'api.anthropic.com', fetchImpl = globalThis.fetch, clock = () => Date.now(), log = () => {} }) {
  ...
  async function post(path, body, { signal, timeoutMs, expectBytes = MAX_RESPONSE_BYTES }) {
    if (!keyPresent) return failure('CREDENTIAL_MISSING', 'no api key supplied (env name resolves to nothing)');
    const controller = new AbortController(); const onAbort = () => controller.abort(); if (signal) { if (signal.aborted) return failure('CANCELLED', 'cancelled before dispatch'); signal.addEventListener('abort', onAbort, { once: true }); }
    ...
    try {
      ...
      catch (err) { if (signal?.aborted) return failure('CANCELLED', 'cancelled', { startedTs }); if (controller.signal.aborted) return failure('TIMEOUT', `no response within ${timeoutMs} ms`, { startedTs, ambiguousCharge: true }); return failure('NETWORK', err?.name ?? 'fetch failed', { startedTs, ambiguousCharge: true }); }
      const requestId = res.headers.get('request-id') ?? res.headers.get('x-request-id') ?? null; const status = res.status;
      if (status >= 300 && status < 400) return failure('NETWORK', 'redirect refused', { status, requestId, startedTs });
      const chunks = []; let total = 0; const reader = res.body?.getReader?.();
      if (reader) { for (;;) { const { done, value } = await reader.read(); if (done) break; total += value.byteLength; if (total > expectBytes) { try { await reader.cancel(); } catch { /* ignore */ } return failure('BODY_TOO_LARGE', 'response exceeds the byte cap', { status, requestId, startedTs, ambiguousCharge: true }); } chunks.push(Buffer.from(value.buffer, value.byteOffset, value.byteLength)); } }
      else { const ab = Buffer.from(await res.arrayBuffer()); if (ab.byteLength > expectBytes) return failure('BODY_TOO_LARGE', 'response exceeds the byte cap', { status, requestId, startedTs }); chunks.push(ab); total = ab.byteLength; }
      ...
      const base = { status, requestId, startedTs, receivedTs, durationMs: receivedTs - startedTs, bytes: total, responseSha256: sha256Hex(bytes), retryAfterMs: status === 429 ? (Number(res.headers.get('retry-after')) || 60) * 1000 : null };
  ...
  async function messages({ body, expectedModel, signal = null, timeoutMs = 90_000 }) {
    ...
    if (!r.ok) return r;
    const j = r.json; if (!j || typeof j !== 'object' || j.type !== 'message' || !Array.isArray(j.content)) return { ok: false, failure: { kind: 'SCHEMA', reason: 'response is not a message object', requestId: r.requestId, status: r.status, startedTs: r.startedTs, receivedTs: r.receivedTs } };
  ...
  async function countTokens({ body, signal = null, timeoutMs = 30_000 }) {
    ...
    if (!r.ok) return r; const n = num(r.json?.input_tokens); return n === null ? { ok: false, failure: { kind: 'SCHEMA', reason: 'input_tokens missing' } } : { ok: true, inputTokens: n, requestId: r.requestId };
  }
