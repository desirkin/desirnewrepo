...
import { buildRequestBody, estimateTokensFromBytes, PROMPT_VERSION } from './prompt.js';
import { createAnthropicClient, parseModelJson } from './client.js';
import { renderReport, REPORT_RENDERER_VERSION } from './report.js';
import { openBudgetJournal, estimateCostUsd, actualCostUsd } from './budget.js';
...
export const ATTEMPT_FAILURE_KINDS = Object.freeze(['MODEL_DISABLED', 'CREDENTIAL_MISSING', 'INPUT_TOO_LARGE', 'BUDGET_BLOCKED', 'RESERVATION_FAILED', 'RECORDED_RESPONSE_MISSING', 'RECORDED_RESPONSE_MISMATCH', 'REPLAY_LIVE_MODEL_OFF', 'DEADLINE', 'CANCELLED', 'OUTPUT_TOO_LARGE', 'JSON_INVALID', 'SCHEMA', 'TRANSPORT', 'REFUSAL', 'TRUNCATED', 'NO_TEXT', 'MODEL_MISMATCH']);
...
export function createCaseRuntime({ policy, env = {}, owner = null, clock = () => Date.now(), log = () => {}, fetchImpl = globalThis.fetch, budgetDir = null, budgetJournal = null, recordedResponse = null, contextRebuilder = null, socialProjectionOf = null, requestCache = null, identity = null, mode = null, timers = { setTimeout, clearTimeout }, maxPendingCases = null } = {}) {
  ...
  const ensureJournal = () => { if (journal) return journal; if (!budgetDir) fail('PERMISSION_FAILURE', 'a live model dispatch needs a budget journal directory (budgetDir) — none configured'); journal = openBudgetJournal({ dir: budgetDir, clock }); ownsJournal = true; return journal; };

  // ---- one model inference attempt --------------------------------------------------------------------------------
  async function attempt({ caseId, attemptNo, packet, subjectRefs, priorAnalysis, brokerResults, revision, deadlineTs, signal, reevaluation = false }) {
    ...
    if (signal?.aborted) return failure('CANCELLED', 'cancelled before dispatch', { path: 'NONE' });
    ...
    if (hit) { const r = accept(hit.text, 'REUSED', { reused: { originalAttemptId: hit.attemptId, originalCreatedTs: hit.createdTs, originalPath: hit.path, actualModel: hit.actualModel, usage: hit.usage, ageMs: clock() - hit.createdTs }, usage: { inputTokens: 0, outputTokens: 0, cacheCreationInputTokens: 0, cacheReadInputTokens: 0 }, estimatedUsd: 0, actualModel: hit.actualModel }); return r; }
    ...
    if (recordedResponse !== null) {
      ...
      if (r.ok) cache.put(requestId, { text, attemptId, createdTs: r.finishedTs, path: 'RECORDED_RESPONSE', actualModel: r.actualModel, usage: r.usage });
      return r;
    }
    ...
    let inputTokens = estimateTokensFromBytes(built.bytes); let tokenCountSource = 'ESTIMATE_BYTES_DIV_3';
    const remainingMs = () => deadlineTs - clock();
    try { const cnt = await c.countTokens({ body: built.body, signal, timeoutMs: Math.max(1000, Math.min(15_000, remainingMs())) }); if (cnt.ok) { inputTokens = cnt.inputTokens; tokenCountSource = 'PROVIDER_COUNT'; } } catch { /* estimate stands */ }
    const estimatedUsd = estimateCostUsd({ inputTokens, maxOutputTokens: modelCfg.maxOutputTokens, pricing: modelCfg.pricing });
    ...
    if (!reserved.ok) return failure('BUDGET_BLOCKED', `budget caps refuse the reservation (${reserved.reasons.join(',')})`, { path: 'NONE', budget: { reasons: reserved.reasons, totals: reserved.totals, estimatedUsd } });
    if (clock() > deadlineTs) { j.releaseReservation({ reservationId, reason: 'DEADLINE_BEFORE_DISPATCH' }); return failure('DEADLINE', 'case deadline reached before dispatch', { path: 'NONE' }); }
    if (signal?.aborted) { j.releaseReservation({ reservationId, reason: 'CANCELLED_BEFORE_DISPATCH' }); return failure('CANCELLED', 'cancelled before dispatch', { path: 'NONE' }); }
    modelInFlight += 1; let res;
    try { res = await c.messages({ body: built.body, expectedModel: modelCfg.model, signal, timeoutMs: Math.max(1000, Math.min(modelCfg.attemptTimeoutMs, remainingMs())) }); } finally { modelInFlight -= 1; }
    const meta = { reservation: { reservationId, estimatedUsd, inputTokens, tokenCountSource, maxOutputTokens: modelCfg.maxOutputTokens } };
    if (!res.ok) {
      const f = res.failure; const known4xx = ['HTTP_401', 'HTTP_403', 'HTTP_429', 'HTTP_4XX'].includes(f.kind);
      let settledUsd = null;
      if (f.kind === 'CANCELLED' && !f.startedTs) j.releaseReservation({ reservationId, reason: 'CANCELLED_BEFORE_DISPATCH' }); // provably never dispatched
      else if (f.usage && Number.isSafeInteger(f.usage.inputTokens)) settledUsd = j.settle({ reservationId, usage: f.usage, pricing: modelCfg.pricing }).actualUsd; // refusal / truncation / mismatch: usage is known and billed
      else if (known4xx) settledUsd = j.settle({ reservationId, usage: { inputTokens: 0, outputTokens: 0, cacheCreationInputTokens: 0, cacheReadInputTokens: 0 }, pricing: modelCfg.pricing }).actualUsd; // provider answered with an error: no tokens processed
      else j.markUnresolved({ reservationId, reason: f.kind }); // timeout / network / 5xx / oversized / invalid body: a charge is not disproven, no automatic refund
      ...
      meta.actualUsd = settledUsd; meta.usage = f.usage ?? null; meta.estimatedUsd = estimatedUsd; meta.actualModel = f.actualModel ?? null; meta.latencyMs = f.durationMs ?? null;
      return failure(kind, `${f.kind}: ${f.reason}`, { path: pathLabel, ...meta, provider: { kind: f.kind, status: f.status ?? null, requestId: f.requestId ?? null, actualModel: f.actualModel ?? null, stopReason: f.stopReason ?? null, usage: f.usage ?? null, durationMs: f.durationMs ?? null, ambiguousCharge: f.ambiguousCharge === true } });
    }
    const settled = j.settle({ reservationId, usage: res.usage, pricing: modelCfg.pricing });
    const usageRec = { usage: res.usage, estimatedUsd, actualUsd: settled.actualUsd, actualModel: res.actualModel, latencyMs: res.durationMs, provider: { requestId: res.requestId, responseId: res.responseId, stopReason: res.stopReason, nonTextBlocks: res.nonTextBlocks, responseBytes: res.responseBytes, responseSha256: res.responseSha256 } };
    ...
    if (r.ok) cache.put(requestId, { text: res.text, attemptId, createdTs: r.finishedTs, path: pathLabel, actualModel: res.actualModel, usage: res.usage });
    return r;
  }

  // ---- follow-up: resolve the model's data requests through the bounded broker, then build P1 at Q1 --------------------
  async function followup({ caseId, analysis, packet, caseSubject, marketContext, socialProjection, deadlineTs, signal, alreadyRequested }) {
    const requests = analysis.dataRequests.slice(0, cases.maxDataRequestsPerCase); const results = [];
    for (const r of requests) { if (signal?.aborted) break; const res = await broker.resolve(r, { analysisId: analysis.analysisId, caseSubject, asOfTs: packet.asOfTs, deadlineTs, alreadyRequested, signal }); alreadyRequested.add(res.requestId); results.push(res); }
    const useful = results.filter((x) => (x.state === 'SATISFIED' || x.state === 'PARTIAL') && x.observationsAdmitted > 0);
    if (!useful.length) return { requests, results, packet: null, reason: 'NO_USEFUL_RESULT' };
    ...
    if (contextRebuilder) { try { const c = await contextRebuilder({ asOfTs: q1, canonicalCoin: caseSubject.canonicalCoin, previous: marketContext, results }); if (c && !contextError(c)) { ctx = c; rebuilt = true; } } catch (err) { log(`context rebuild failed: ${String(err?.message ?? err).slice(0, 120)}`); } }
    ...
    const b = buildResearchEvidenceV2({ marketContext: ctx, socialProjection: social, asOfTs: q1, trigger: packet.trigger, mode: caseMode, entrances: packet.researchContext.entrances, detailRequests, coverageLimitations: rebuilt ? [] : [] });
    if (!b.ok) return { requests, results, packet: null, reason: `PACKET_BUILD_${b.reason}`, detail: b.detail };
    if (b.packet.packetId === packet.packetId) return { requests, results, packet: null, reason: 'PACKET_UNCHANGED' };
    return { requests, results, packet: b.packet, contextMap: b.contextMap, reason: 'NEW_PACKET', q1, rebuilt };
  }

  // ---- the case ------------------------------------------------------------------------------------------------------------
  function runCase({ packet, marketContext = null, socialProjection = null, registeredRefs = [], out = null, signal = null, reevaluation = false, contextMap = null, trigger = null } = {}) {
    ...
    const run = async () => {
      try {
        ...
        if (signal?.aborted) return finish('CANCELLED', { kind: 'CANCELLED' });
        ...
        if (!a1.ok) { const k = a1.failure.kind; return finish(k === 'BUDGET_BLOCKED' || k === 'MODEL_DISABLED' || k === 'CREDENTIAL_MISSING' || k === 'RESERVATION_FAILED' ? 'BUDGET_BLOCKED' : k === 'DEADLINE' ? 'DEADLINE_EXCEEDED' : k === 'CANCELLED' ? 'CANCELLED' : 'MODEL_FAILED', { kind: k, reason: a1.failure.reason, attemptId: a1.attemptId, path: a1.path }); }
        ...
        followupInfo = { round: 1, requested: f.requests.length, resolved: f.results.length, states: Object.fromEntries(f.results.map((x) => [x.requestKey, x.state])), outcome: f.reason, q1: f.q1 ?? null, contextRebuilt: f.rebuilt === true, detail: f.detail ?? null };
        if (signal?.aborted) return finish('CANCELLED', { kind: 'CANCELLED', note: 'cancelled during follow-up acquisition; the initial report is recorded, not published as final' });
        ...
        if (!a2.ok) return finish('INITIAL_REPORT_ONLY', { kind: a2.failure.kind, reason: a2.failure.reason, attemptId: a2.attemptId, path: a2.path, followupFailure: true });
    ...
    handle.done = run().then((r) => (out ? { ...r, bundle: sealCase(r, out) } : r));
  ...
  function close() { if (ownsJournal && journal) { journal.close(); journal = null; ownsJournal = false; } for (const h of active.values()) h.cancelled = true; }
  return { runCase, sealCase, attempt, close, broker, cache, status: () => ({ runtimeVersion: RUNTIME_VERSION, mode: caseMode, activeCases: active.size, pendingLimit, modelInFlight, cacheSize: cache.size(), model: { enabled: modelCfg.enabled, model: modelCfg.model, credentialPresent: typeof apiKey === 'string' && apiKey.length > 0, capsPositive: modelCfg.maxEstimatedUsdPerCase > 0 && modelCfg.maxEstimatedUsdPerDay > 0 && modelCfg.maxEstimatedUsdPerMonth > 0 }, budget: journal ? journal.totals() : null }), caseIdentity: (p) => caseIdentity({ packetId: p.packetId, policyDigest: pd, mode: caseMode, trigger: p.trigger?.kind ?? null }) };
}

// ---- default live context rebuilder over a research owner (P1 at Q1 from the owner's retained observations) -----------------
export function ownerContextRebuilder(owner, { referenceNotionals = [1000, 10000, 100000], peers = [], limits = RESOURCE_DEFAULTS } = {}) {
  return async ({ asOfTs, canonicalCoin, previous }) => {
    const obs = owner.observations(); const cov = owner.coverage();
    const captureRef = previous?.captureRef ?? { bundleId: 'live-owner', manifestSha256: 'live', observationsSha256: 'live', coverageSha256: 'live' };
    const b = buildContext({ canonicalCoin, asOfTs, observations: obs, coverage: cov, captureRef, referenceNotionals, peers: peers.length ? peers : [...new Set(obs.filter((o) => o.kind === 'TRADE' && o.subject.canonicalCoin !== canonicalCoin).map((o) => o.subject.canonicalCoin))].sort(), limits, resourceState: owner.hot?.status?.() ?? null });
    return b.context;
  };
}

// ---- offline verification of a sealed case (§12 verify): members, hashes, contexts, citations, clocks, report, usage ------
export function verifyCase(dir, { limits = RESOURCE_DEFAULTS } = {}) {
  ...
  return { ok: reasons.length === 0, reasons, caseId: manifest.caseId ?? null, status: manifest.status ?? null, packets: packets.length, analyses: analyses.length, attempts: usage.length, bundleId: bundle.manifest.bundleId, note: 'checksums prove internal consistency, not source authenticity or model billing correctness' };
}
