...
// venue is simulated, and the simulation is declared conservative. Simulated arrival = dispatch + latency; the latency
// is the LIVE-MEASURED Kraken round-trip from the gateway collector when a latencySource is injected (widened when Kraken
// reports degraded — see lib/paper-fill-latency.js), else the conservative reference (250ms). The fill then samples the
// FIRST accepted executable book received at/after arrival within at most 1000ms (its
...
export function createPaperAdapter({ accountId, clock, feed = null, fee, specOf, latencyMs = PAPER_DEFAULTS.latencyMs, maxObservationWaitMs = PAPER_DEFAULTS.maxObservationWaitMs, latencySource = null, script = () => ({}), restore = null, log = () => {} } = {}) {
  // The effective fill delay: the injected live measurement when present and sane, else the fixed conservative fallback.
  // A source that returns a non-finite / non-positive value is treated as "no measurement" — the sim never fills faster
  // than the fallback on a bad reading. Read per fill so a mid-run degradation widens the very next arrival.
  const arrivalLatency = (fallback) => { if (typeof latencySource !== 'function') return fallback; let ms; try { ms = latencySource(); } catch { return fallback; } return typeof ms === 'number' && Number.isFinite(ms) && ms > 0 ? ms : fallback; };
  ...
  const adapter = {
    ...
    async submitEntryWithProtection({ intent, spec: givenSpec }) {
      ...
      const nativeOrderId = `pord-${accountId}-${++seq}`; const order = { kind: 'ENTRY', intent, spec, arrivalTs: now + arrivalLatency(latencyMs), submittedTs: now, nativeOrderId, resolved: false }; pending.set(intent.orderId, order); orders.set(intent.orderId, order);
