...
const MAX_RECENT = 256; const MAX_EXTERNAL = 64; const MAX_EXEC_IDS = 4096;
...
export function emptyAccountState(accountId) {
  return { stateVersion: ACCOUNT_STATE_VERSION, accountId, initialized: false, accountKind: null, mode: 'OBSERVE', venue: null, quote: null, initialCapital: null, policyDigest: null, policyVersion: null, ownerRef: null, limits: null, compounding: null,
    cash: '0', externalFlows: '0', flowsUnknown: false, reservations: {}, orders: {}, positions: {}, hypotheses: {}, decisions: {}, decisionOrder: [], execIds: {}, execOrder: [], externalActivity: [], authorization: null, authorizations: {},
    ...
    performance: { highWater: null, riskPerformanceEquity: null, drawdown: null, session: null }, realized: { pnl: '0', fees: '0', baseFees: {} }, lastReconciliation: null, clockAnchor: null, pins: {},
...
const HANDLERS = {
  ...
  ACCOUNT_AUTHORIZED(s, p, ev) {
    ...
    const a = { ...p, ended: false, endedReason: null }; s.authorizations[p.authorizationId] = a; s.authorization = a;
    if (p.limits) s.limits = p.limits;
  },
  AUTHORIZATION_ENDED(s, p) { const a = s.authorizations[p.authorizationId] ?? refuse('AUTHORIZATION_UNKNOWN', p.authorizationId); a.ended = true; a.endedReason = p.reason; if (s.authorization?.authorizationId === p.authorizationId) { s.authorization = a; if (s.mode === 'LIVE_ARMED') s.mode = 'REDUCE_ONLY'; } },
  ...
  ORDER_INTENT(s, p, ev) {
    ...
    if (p.kind === 'ENTRY' || p.kind === 'CANARY_ENTRY') {
      ...
      if (p.kind === 'PROTECTIVE_STOP') { pos.protection = { ...pos.protection, state: 'PENDING', orderId: p.orderId, trigger: p.limitPrice, qty: p.qty }; } else pos.state = 'EXITING';
    }
  ...
  EXECUTION_RECORDED(s, p, ev) {
    ...
    if (o.kind === 'PROTECTIVE_STOP') pos.protection = { ...pos.protection, state: M.eq(totalFilled, o.qty) ? 'TRIGGERED' : pos.protection.state, filledBase: totalFilled };
    ...
    s.execIds[key] = content; pushBounded(s.execOrder, key, MAX_EXEC_IDS); for (const k of Object.keys(s.execIds)) if (!s.execOrder.includes(k) && s.execOrder.length >= MAX_EXEC_IDS) delete s.execIds[k];
  ...
  CANCEL_RESULT(s, p) {
    ...
    if (p.outcome === 'CANCELLED') { o.state = TERMINAL_ORDER_STATES.includes(o.state) ? o.state : 'CANCELLED'; if (o.terminalTs === null) { o.terminalTs = p.receiptTs; settleEntryTerminal(s, o); } if (o.kind === 'PROTECTIVE_STOP') pos.protection = { ...pos.protection, state: 'CANCELLED' }; }
    else if (p.outcome === 'REJECTED') { o.state = o.priorState ?? 'ACKNOWLEDGED'; if (o.kind === 'PROTECTIVE_STOP') pos.protection.state = 'ACTIVE'; }
    else if (p.outcome === 'ALREADY_TERMINAL') { if (!TERMINAL_ORDER_STATES.includes(o.state)) { o.state = 'RECONCILIATION_REQUIRED'; s.restrictions.RECONCILIATION_REQUIRED = { ts: p.receiptTs, source: 'reducer', reason: `order ${p.orderId} terminal at venue but not recorded` }; if (o.kind === 'PROTECTIVE_STOP') pos.protection.state = 'MISMATCH'; } }
    else { o.state = 'RECONCILIATION_REQUIRED'; s.restrictions.RECONCILIATION_REQUIRED = { ts: p.receiptTs, source: 'reducer', reason: `cancel of ${p.orderId} uncertain` }; if (o.kind === 'PROTECTIVE_STOP') pos.protection.state = 'MISMATCH'; }
  },
  PROTECTION_STATE(s, p) {
    const pos = positionOf(s, p.positionId); const prot = pos.protection;
    if (p.state === 'ACTIVE') { if (p.qty === null || p.trigger === null) refuse('PROTECTION_FACTS', 'ACTIVE needs confirmed qty and trigger'); if (!M.isZero(ownedBase(pos)) && M.lt(p.qty, ownedBase(pos))) { pos.protection = { ...prot, state: 'MISMATCH', orderId: p.orderId ?? prot.orderId, nativeOrderId: p.nativeOrderId ?? prot.nativeOrderId, trigger: p.trigger, qty: p.qty, confirmedTs: p.receiptTs, mismatch: `covers ${p.qty} of ${ownedBase(pos)}` }; s.restrictions.PROTECTION_MISMATCH = { ts: p.receiptTs, source: 'reducer', reason: `position ${p.positionId} protection covers ${p.qty} of ${ownedBase(pos)}` }; return; } if (prot.trigger && M.lt(p.trigger, prot.trigger) && prot.state === 'ACTIVE') refuse('PROTECTION_LOOSENED', `trigger ${p.trigger} below confirmed ${prot.trigger}`); }
    ...
    pos.protection = { ...prot, state: p.state, orderId: p.orderId ?? prot.orderId, nativeOrderId: p.nativeOrderId ?? prot.nativeOrderId, trigger: p.trigger ?? prot.trigger, qty: p.qty ?? prot.qty, confirmedTs: p.state === 'ACTIVE' ? p.receiptTs : prot.confirmedTs, reason: p.reason };
  },
  PROTECTION_AMEND(s, p) {
    const pos = positionOf(s, p.positionId); const prot = pos.protection; if (prot.orderId !== p.orderId) refuse('PROTECTION_ORDER_MISMATCH', 'amend names another order');
    if (p.outcome === 'REQUESTED') { if (prot.state !== 'ACTIVE') refuse('AMEND_NOT_ACTIVE', prot.state); if (prot.pendingAmend) refuse('AMEND_PENDING', 'one unresolved amendment per child stop'); if (prot.trigger && !M.gt(p.requestedTrigger, prot.trigger)) refuse('AMEND_NOT_TIGHTER', `${p.requestedTrigger} does not tighten ${prot.trigger}`); prot.pendingAmend = { amendId: p.amendId, requestedTrigger: p.requestedTrigger, ts: p.receiptTs }; prot.state = 'AMEND_PENDING'; return; }
    const pend = prot.pendingAmend; if (!pend || pend.amendId !== p.amendId) refuse('AMEND_STALE', 'a stale amend response cannot overwrite newer confirmed state');
    if (p.outcome === 'ACKNOWLEDGED') { if (p.confirmedTrigger === null || M.lt(p.confirmedTrigger, prot.trigger ?? '0')) refuse('AMEND_CONFIRMATION', 'acknowledgement must confirm a tighter trigger'); prot.trigger = p.confirmedTrigger; prot.state = 'ACTIVE'; prot.pendingAmend = null; prot.lastAmendTs = p.receiptTs; }
    else if (p.outcome === 'FAILED') { prot.state = 'ACTIVE'; prot.pendingAmend = null; }
    else { prot.state = 'MISMATCH'; prot.pendingAmend = null; s.restrictions.RECONCILIATION_REQUIRED = { ts: p.receiptTs, source: 'reducer', reason: `amend ${p.amendId} uncertain` }; }
  },
  ...
  MODE_TRANSITION(s, p, ev) {
    ...
    if (to === 'LIVE_ARMED') { if (!authorizationActive(s, ev.knownAtTs) || s.authorization.kind !== 'LIVE_ARM') refuse('ARM_REQUIRED', 'LIVE_ARMED needs an active LIVE_ARM authorization'); if (p.authorizationId !== s.authorization.authorizationId) refuse('ARM_BINDING', 'transition must name the active authorization'); const hard = HARD_RESTRICTIONS.filter((c) => s.restrictions[c]); if (hard.length) refuse('ARM_BLOCKED', hard.join(',')); if (s.valuation.unknown) refuse('ARM_BLOCKED', 'valuation unknown'); }
  ...
  FEE_ADJUSTMENT(s, p) { if (!s.execIds[`${s.venue}|${p.execId}`]) refuse('EXEC_UNKNOWN', p.execId); const o = p.orderId ? orderOf(s, p.orderId) : null; const pos = o ? positionOf(s, o.positionId) : null; if (p.asset === s.quote) { s.cash = M.sub(s.cash, p.delta); s.realized.fees = M.add(s.realized.fees, p.delta); if (pos) { if (o.side === 'buy') pos.entryFeesQuote = M.add(pos.entryFeesQuote, p.delta); else pos.exitFeesQuote = M.add(pos.exitFeesQuote, p.delta); } } else if (pos && p.asset === pos.assetId) { pos.baseFees = M.add(pos.baseFees, p.delta); } else { s.flowsUnknown = true; s.valuation = { ...s.valuation, unknown: true, reason: `fee in ${p.asset} needs a known conversion` }; } if (pos) { pos.feeCorrections = [...(pos.feeCorrections ?? []), { execId: p.execId, asset: p.asset, delta: p.delta, ref: p.ref, ts: p.ts }]; recomputeRealized(s, pos); } },
  INVENTORY_ADJUSTMENT(s, p) { if (p.positionId) { const pos = positionOf(s, p.positionId); const next = M.add(ownedBase(pos), p.delta); if (M.isNegative(next)) refuse('INVENTORY_NEGATIVE', 'adjustment would make owned base negative'); pos.confirmedBase = M.add(pos.confirmedBase, p.delta); pos.adjustments = [...(pos.adjustments ?? []), { ...p }]; } else pushBounded(s.externalActivity, { kind: 'UNKNOWN', ref: p.cause, asset: p.asset, amount: p.delta, receiptTs: p.ts, adjustment: true }, MAX_EXTERNAL); },
  DUST_STATE(s, p) { const pos = positionOf(s, p.positionId); if (!M.eq(ownedBase(pos), p.base)) refuse('DUST_MISMATCH', `${ownedBase(pos)} != ${p.base}`); if (p.state === 'WRITTEN_OFF' && !p.ownerRef) refuse('OWNER_WRITEOFF_REQUIRED', 'a write-off is an owner-approved accounting adjustment'); pos.dust = { base: p.base, state: p.state, ownerRef: p.ownerRef, ts: p.ts }; pos.state = p.state === 'WRITTEN_OFF' ? 'FLAT' : 'DUST_UNRESOLVED'; if (p.state === 'WRITTEN_OFF') { pos.writtenOffBase = p.base; pos.baseFees = M.add(pos.baseFees, p.base); pos.riskReserved = '0'; pos.feedPinned = false; } },
...
function recomputeRealized(s, pos) { if (pos.pnlUnknown) { pos.realizedPnl = null; return; } if (!M.isZero(pos.soldBase) || pos.state === 'FLAT') { const bought = pos.confirmedBase; const entryAllocated = M.isZero(bought) ? '0' : M.div(M.mul(M.add(pos.entryQuote, pos.entryFeesQuote), pos.soldBase), bought, 8, 'HALF_UP'); pos.realizedPnl = M.sub(M.sub(pos.exitQuote, pos.exitFeesQuote), entryAllocated); } s.realized.pnl = M.sum(Object.values(s.positions).map((x) => x.realizedPnl ?? '0')); }
