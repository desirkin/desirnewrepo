...
export function buildShadowCapture({ recipe, venue, assetId, decisionTs, inputs }) {
  ...
  const sealedRecipe = recipeSeal.recipe;
  ...
  let opportunityId = opportunityIdOf({ venue, assetId, decisionTs, recipeVersion: sealedRecipe.recipeVersion, recipeDigest: recipeSeal.recipeDigest, windowEndTs: null });
  const refuse = (reason, detail) => deepFreeze({
    eligible: [], ineligible: [{ kind: 'INELIGIBLE_OPPORTUNITY', opportunityId, lane: LANE, venue, assetId, decisionTs, recipeVersion: sealedRecipe.recipeVersion, recipeDigest: recipeSeal.recipeDigest, reason, detail: String(detail).slice(0, 300), authority: AUTHORITY, purpose: PURPOSE }],
  });
  ...
  if (candles.length < sealedRecipe.candleWindowMin) return refuse('CANDLE_WINDOW_INCOMPLETE', `${candles.length}/${sealedRecipe.candleWindowMin} candles`);
  const window = candles.slice(-sealedRecipe.candleWindowMin);
  for (let i = 0; i < window.length; i += 1) {
    ...
    if (c.periodEndTs - c.periodStartTs !== sealedRecipe.candlePeriodMs) return refuse('CANDLE_WINDOW_INCOMPLETE', `candle ${i} is not one declared ${sealedRecipe.candlePeriodMs}ms bar`);
    ...
    if (sealedRecipe.requiredInputs.includes('VOLUME') && !isFiniteNum(c.volumeBase) && !isFiniteNum(c.volumeQuote)) return refuse('VOLUME_MISSING', `candle ending ${c.periodEndTs} carries no real volume component — a required fact is never zero-filled`);
    if (sealedRecipe.requiredInputs.includes('TRADE_FLOW') && !isFiniteNum(c.tradeFlow)) return refuse('TRADE_FLOW_MISSING', `candle ending ${c.periodEndTs} carries no trade-flow`);
  }
  ...
  if (sealedRecipe.requiredInputs.includes('VOLUME') && volumeComponents.length === 0) return refuse('VOLUME_MISSING', 'no volume component is observed across the WHOLE window — a partial series is not a unit');
  const last = window[window.length - 1];
  if (decisionTs - last.periodEndTs > sealedRecipe.maxInputAgeMs) return refuse('REQUIRED_INPUT_STALE', `last completed candle ended ${decisionTs - last.periodEndTs}ms before the decision (max ${sealedRecipe.maxInputAgeMs}ms)`);
  ...
  if (sealedRecipe.requiredInputs.includes('DEPTH_SNAPSHOT') && !depth) return refuse('REQUIRED_INPUT_MISSING', 'the recipe requires a depth snapshot');
  ...
  for (const [kind, value] of [['SOCIAL_CONTEXT', inputs?.social ?? null], ['NEWS_CONTEXT', inputs?.news ?? null]]) {
    if (sealedRecipe.requiredInputs.includes(kind) && !value) return refuse('REQUIRED_INPUT_MISSING', `the recipe requires ${kind}`);
  ...
  const inputDigest = canonicalDigest({ lane: LANE, venue, assetId, decisionTs, recipeVersion: sealedRecipe.recipeVersion, recipeDigest: recipeSeal.recipeDigest, costPolicyVersion: sealedRecipe.costPolicy.costPolicyVersion, frozenInputs });
  ...
  const inputUnits = { price: 'QUOTE_PER_BASE', volumeComponents, tradeFlow: 'SIGNED_FRACTION', candlePeriodMs: sealedRecipe.candlePeriodMs };
  ...
  opportunityId = opportunityIdOf({ venue, assetId, decisionTs, recipeVersion: sealedRecipe.recipeVersion, recipeDigest: recipeSeal.recipeDigest, windowEndTs: last.periodEndTs });
  const eligible = sealedRecipe.variants.map((variant) => {
    const record = {
      ...
      groupId: decisionMomentIdOf({ venue, assetId, decisionTs }), lane: LANE,
      venue, assetId, decisionTs,
      recipeVersion: sealedRecipe.recipeVersion, recipeDigest: recipeSeal.recipeDigest, recipeSeal,
      costPolicyVersion: sealedRecipe.costPolicy.costPolicyVersion,
      inputDigest,
