...
export function createShadowLane({ store, recipe, quotas = {}, clock = () => Date.now(), monotonic = () => performance.now(), log = () => {} }) {
  ...
  const activeRecipeSeal = sealShadowRecipe(recipe);
  const activeRecipe = activeRecipeSeal.recipe;
  const priorDigest = typeof store?.recipeDigestFor === 'function' ? store.recipeDigestFor(activeRecipe.recipeVersion) : null;
  if (priorDigest !== null && priorDigest !== activeRecipeSeal.recipeDigest) throw new Error(`shadow lane: RECIPE_VERSION_CONFLICT for ${activeRecipe.recipeVersion}`);
  ...
  function runBatch({ opportunities, nowTs = clock() }) {
    ...
    for (const opp of slice) {
      ...
      let oppShed = false;
      let built;
      try { built = buildShadowCapture({ recipe: activeRecipe, venue: opp.venue, assetId: opp.assetId, decisionTs: opp.decisionTs, inputs: opp.inputs }); }
  ...
  function matureBatch({ paths, asOfTs = clock() }) {
    ...
    for (const [captureId, capture] of store.captures()) {
      ...
      processed += 1;
      let outcome;
      if (!capture.recipeSeal?.recipeDigest) { out.errors += 1; log(`shadow lane: legacy unsealed capture ${captureId} remains read-only and unmatured`); continue; }
      try { outcome = matureShadowCapture({ capture, costPolicy: capture.recipeSeal.recipe.costPolicy, horizonMin: capture.recipeSeal.recipe.horizonMin, path: path.candles, asOfTs, depthPath: path.depthPath ?? null }); }
  ...
  function status(nowTs = clock()) {
    ...
    return deepFreeze({
      ...
      activeRecipe: { recipeVersion: activeRecipe.recipeVersion, recipeDigest: activeRecipeSeal.recipeDigest },
      journal: s,
      quotas: Q,
