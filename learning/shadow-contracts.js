...
export const SHADOW_CAPTURE_VERSION = 'forward-shadow-capture-1';
...
export const CAPTURE_KEYS = Object.freeze([
  ...
  'decisionTs', 'recipeVersion', 'costPolicyVersion', 'inputDigest', 'inputWindow', 'inputFidelity',
...
export function recipeError(r) {
  ...
  for (const f of ['feePctPerSide', 'assumedHalfSpreadBps', 'assumedLatencyMs']) if (!isFiniteNum(r.costPolicy[f]) || r.costPolicy[f] < 0) return `recipe: costPolicy ${f} malformed`;
  ...
  for (const v of r.variants) {
    ...
    if (v.limitOffsetBps !== null && (!isFiniteNum(v.limitOffsetBps) || v.limitOffsetBps < 0)) return 'recipe: variant limitOffsetBps malformed';
    if (v.decision === 'TAKE') { for (const f of ['stopPct', 'targetPct']) if (!isFiniteNum(v[f]) || v[f] <= 0) return `recipe: variant ${f} malformed`; }
  }
