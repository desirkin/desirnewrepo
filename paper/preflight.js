...
export async function runPreflight({ profileFile = process.env.COBRA_PROFILE ?? 'config/paper-runtime.json', env = process.env, smoke = false, now = () => Date.now(), log = () => {}, dbFactory = null, fetchImpl = globalThis.fetch } = {}) {
  ...
  // Only the KEPT market providers own a senses/preflight row (SENSE-CULL-2): SETTLED_RECORDS is a read-only RUMOR2
  // projection wired in the owner, not a market-research row, so the snapshot emits none for it — filtering to
  // KEPT_MARKET_PROVIDERS keeps the F line in lock-step with the snapshot instead of rendering an empty (undefined) row.
  const mrRows = Object.keys(profile.groups.marketResearch.providers).filter((id) => KEPT_MARKET_PROVIDERS.includes(id)).map((id) => { const r = row(id); const pol = mrPolicy?.providers?.[id] ?? null; return { ...r, policyEnabled: pol?.enabled ?? null, billing: pol?.plan?.billing ?? null, credentialEnv: pol?.credentialEnv ?? null, credentialPresent: pol?.credentialEnv ? present(env, pol.credentialEnv) : null, access: presence?.[id]?.access ?? null, smokeProbe: probeOf(id) }; });
  ...
  for (const r of mrRows) { if (r.state === 'BLOCKED_BUDGET') block('PAID_SENSE_NOT_AUTHORIZED', r.id, `${r.id}: ${r.blocker}`, 'attest plan + billing in the policy (plan.attestation, smoke.maxEstimatedUsd) before any paid call'); else if (r.state === 'BLOCKED_CREDENTIAL') block('EXTERNAL_OPTIONAL_SENSE_BLOCKER', r.id, `${r.id}: ${r.blocker}`); }
  // ---- G. DARK EDGE CAPTURE ---------------------------------------------------------------------------------------------------------
