...
export async function runPreflight({ profileFile = process.env.COBRA_PROFILE ?? 'config/paper-runtime.json', env = process.env, smoke = false, now = () => Date.now(), log = () => {}, dbFactory = null, fetchImpl = globalThis.fetch } = {}) {
  ...
  const socialRows = ['BLUESKY_OFFICIAL', 'X_OFFICIAL', 'FARCASTER_OFFICIAL', 'REDDIT_OFFICIAL', 'STOCKTWITS_OFFICIAL', 'META_PUBLIC', 'TIKTOK_PUBLIC'].map((id) => { const r = row(id); const m = (Array.isArray(matrix?.providers) ? matrix.providers : []).find((p) => p.provider === id) ?? null; return { ...r, matrixReadiness: m?.readiness ?? null, matrixBlockers: m?.blockers ?? [], ...(id === 'X_OFFICIAL' ? { governor: xState, governorDetail: xState === 'READY_REQUIRES_EXPLICIT_PAID_SMOKE' ? 'credential + budgets present; an EXPLICIT paid smoke envelope is still the law before the stream connects' : xState } : {}) }; });
