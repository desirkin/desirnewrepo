...
export function createYouTubeClient({
  ...
  now = () => Date.now(),
  // Optional durable quota adapter. When supplied, it must return a
  // reservation and the caller must append/commit it before dispatch. The
  // legacy fixture path remains available for deterministic offline tests.
  quotaAdmission = null,
} = {}) {
  ...
  async function poll({
    ...
    for (let page = 0; page < maxPages; page += 1) {
      ...
      if (quotaAdmission) {
        if (typeof quotaAdmission.reserve !== 'function' || typeof quotaAdmission.append !== 'function' || typeof quotaAdmission.commit !== 'function') {
          return failedPoll('DURABLE_QUOTA_ADMISSION_INVALID', { provider: YOUTUBE_OFFICIAL.id, retrievedTs, requests });
        }
        const admission = quotaAdmission.reserve({ knownAtTs: retrievedTs, units: YOUTUBE_QUOTA_UNIT_COST });
        if (!admission?.ok || !admission.event) {
          return failedPoll(admission?.reason ?? 'DURABLE_QUOTA_ADMISSION_REQUIRED', { provider: YOUTUBE_OFFICIAL.id, retrievedTs, requests });
        }
        const persisted = await quotaAdmission.append(admission.event);
        if (!persisted?.ok) return failedPoll(persisted?.error ?? 'DURABLE_QUOTA_RESERVATION_UNPERSISTED', { provider: YOUTUBE_OFFICIAL.id, retrievedTs, requests });
        const committed = quotaAdmission.commit(admission.event);
        if (!committed?.ok) return failedPoll(committed.error ?? 'DURABLE_QUOTA_RESERVATION_UNCOMMITTED', { provider: YOUTUBE_OFFICIAL.id, retrievedTs, requests });
        lastQuotaReceipt = admission.event;
      }
    ...
    const common = {
      ...
      ...(quotaAdmission ? { durableQuota: { reservations: requests, lastReservation: lastQuotaReceipt } } : {}),
    };
