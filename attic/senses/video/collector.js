...
import { appendJsonl, atomicWriteJson as writeAtomicJson, readJsonlTail, readJsonBounded, replaceJsonl } from '../lib/jsonl.js';
...
export function videoConfigFromEnv(env = process.env, src = YOUTUBE_DATA_API) {
  ...
  const queries = csv(env[src.queriesEnv]); const rawBudget = env[src.budgetEnv];
...
export function videoGate(cfg, src = YOUTUBE_DATA_API) {
  ...
  if (cfg.queries.some(q => q.length > src.limits.maxQueryChars)) return { ok: false, reason: 'CONFIG_REQUIRED', detail: 'a query exceeds the documented bound' };
...
export function startVideo({ env = process.env, dataDir = defaultDataDir(), fetchImpl = fetch, clock = () => Date.now(), log = console.log, source = YOUTUBE_DATA_API, timers = { setTimeout, clearTimeout, setInterval, clearInterval }, firstDelayMs = VIDEO_RUNTIME.firstDelayMs, signals = true, durableCheckpoint = null } = {}) {
  ...
  async function poll() {
    ...
    try {
      ...
      const nextPageToken = r.json?.nextPageToken ?? null;
      if (parsed.rejected || r.json.items.length > source.limits.maxResultsPerSearch || (nextPageToken !== null && (typeof nextPageToken !== 'string' || !nextPageToken || nextPageToken.length > 2048 || nextPageToken === pageToken))) { st.counters.parseFailed += 1; fail(receiptTs, 'PARSE_FAILED', 'search page incomplete or invalid'); return { outcome: 'PARSE_FAILED', admitted: 0 }; }
