...
export const SANTIMENT_METRICS = deepFreeze({
  ...
  // JUDGE-1 §5.2: documented large-transfer metrics (restricted access; a free key is not evidence of entitlement). Aggregate large
  // transfers WITHOUT entity attribution; the 1m group is nested inside the 100k group. Native interval 5m (also 1h / 1d).
  whale_transaction_count_100k_usd_to_inf: { metric: 'whale_transaction_count_100k_usd_to_inf', unit: 'COUNT', family: 'NETWORK_ACTIVITY', entity: null, intervals: ['5m', '1h', '1d'] },
  whale_transaction_count_1m_usd_to_inf: { metric: 'whale_transaction_count_1m_usd_to_inf', unit: 'COUNT', family: 'NETWORK_ACTIVITY', entity: null, intervals: ['5m', '1h', '1d'] },
  whale_transaction_volume_100k_usd_to_inf: { metric: 'whale_transaction_volume_100k_usd_to_inf', unit: 'USD', family: 'NETWORK_ACTIVITY', entity: null, intervals: ['5m', '1h', '1d'] },
  whale_transaction_volume_1m_usd_to_inf: { metric: 'whale_transaction_volume_1m_usd_to_inf', unit: 'USD', family: 'NETWORK_ACTIVITY', entity: null, intervals: ['5m', '1h', '1d'] },
});
export const SANTIMENT_INTERVALS = Object.freeze({ '5m': { stepMs: 300_000, window: 'five_minutes' }, '1h': { stepMs: HOUR_MS, window: 'hour' }, '1d': { stepMs: DAY_MS, window: 'day' } });
...
export function createSantimentClient({ transport, clock, log, credential = null } = {}) {
  ...
  async function series({ slug, canonicalCoin, metricId, fromTs, toTs, interval = '1d', signal }) {
    ...
    const allowed = m?.intervals ?? ['1d', '1h']; if (!m || !SLUG_RE.test(slug ?? '') || !allowed.includes(interval)) return { ok: false, failure: { kind: 'SCHEMA', reason: 'unsupported slug/metric/interval', coverageState: 'NOT_SUPPORTED', reasonCode: 'NONE', ts: base.clock() }, coverage: [] };
    ...
    const step = SANTIMENT_INTERVALS[interval].stepMs; const windowLabel = SANTIMENT_INTERVALS[interval].window; const out = [];
    for (const raw of rows) { const row = obj(raw); const start = tsFromIso(row?.datetime); const v = num(row?.value); if (start === null) continue; const end = start + step;
      const ob = base.tryEmit({ endpointId: 'graphql-get-metric', subject, kind: 'ONCHAIN_METRIC', sourceKey: `${slug}:${m.metric}:${interval}:${start}`, periodStartTs: start, periodEndTs: end, receivedTs: r.receivedTs, knownAtTs: r.receivedTs, quality: quality(v === null ? 'MISSING' : end > r.receivedTs ? 'PARTIAL' : 'KNOWN', { reasonCodes: v === null ? ['FIELD_MISSING_AT_SOURCE'] : end > r.receivedTs ? ['UNCOMMITTED_BAR'] : [], coverageStartTs: start, coverageEndTs: Math.min(end, r.receivedTs), methodologyId: `santiment-${m.metric}-v1`, originalUnit: m.unit }), provenance: base.provenance(r, { nativeLocator: `${slug}/${m.metric}/${start}`, mappingId: SANTIMENT_MAPPING_ID, vintage: String(r.receivedTs) }), payload: { metricId, value: v, unit: m.unit, entitySet: m.entity, chain: slug === 'bitcoin' ? 'bitcoin' : slug === 'ethereum' ? 'ethereum' : slug, window: windowLabel, methodologyId: `santiment-${m.metric}-v1`, labelVintage: m.entity ? `santiment-labels:${new Date(r.receivedTs).toISOString().slice(0, 10)}` : null } });
      if (ob) out.push(ob); }
