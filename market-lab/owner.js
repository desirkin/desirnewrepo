...
export const NETWORK_NATIVE_IDS = deepFreeze({ active_addresses: ['active_addresses'], transaction_count: ['transaction_count'], transfer_volume: ['transfer_volume'], network_fees: ['fees_total'], realized_value_metrics: ['mvrv', 'sopr', 'realized_price'] });
...
export function createResearchOwner({ policy, subjects, env = {}, researchRoot = null, mode = 'STANDALONE', clock = () => Date.now(), log = () => {}, transport = null, fetchImpl = globalThis.fetch, WebSocketImpl = globalThis.WebSocket, wsUrls = {}, settledAccessors = {}, socialProjection = null, timers = { setInterval, clearInterval, setTimeout, clearTimeout }, recorder = null, quotaJournal = null, onRecordingFailure = null } = {}) {
  ...
  const record = (obs) => { for (const o of obs) { if (recording.error) return; if (duplicate(o)) { counters.duplicateRecords += 1; continue; } if (!persist('obs', o)) { if (recording.error) return; continue; } if (segment) noteSegmentObs(o); counters.observations += 1; const evictions = retained.push(o); if (evictions.length) { counters.evictedObservations += evictions.reduce((n, c) => n + c.droppedCount, 0); recordCoverage(evictions); } } };
  const recordCoverage = (cov) => { for (const c of cov) { if (recording.error) return; if (!persist('cov', c)) continue; if (segment) segment.stats.coverage += 1; counters.coverageRecords += 1; retained.pushCoverage(c); } };
  ...
  async function acquire(family, coin, { signal = null, force = false, metricIds = null, windowStartTs = null, windowEndTs = null } = {}) {
    ...
    const run = async (id, endpointId, key, fn, { ttlMs = null } = {}) => {
      ...
      counters.acquisitions += 1; let r; const u0 = usageSnapshot();
    ...
    switch (family) {
      ...
      case 'NETWORK_ACTIVITY': {
        ...
        else if (missing.length && s?.santiment && enabled('SANTIMENT')) { for (const mid of missing.filter((x) => SANTIMENT_METRICS[x] && SANTIMENT_METRICS[x].family === 'NETWORK_ACTIVITY')) { const r = await run('SANTIMENT', 'graphql-get-metric', `sann:${coin}:${mid}:${windowStartTs ?? ''}`, () => clients.SANTIMENT.series({ slug: s.santiment, canonicalCoin: coin, metricId: mid, fromTs: histFrom, toTs: histTo, signal }), { ttlMs: HOUR_MS }); if (r === null && out.results[out.results.length - 1]?.state === 'QUOTA_REFUSED') break; } }
        break; }
  ...
  async function stopInternal({ seal, policyNonsecret, reason }) {
    ...
    stopResult = { sealed: sealedNow, segments: sealed.slice(), error: recording.error ? { ...recording.error, where: recording.where, failedTs: recording.failedTs } : null, streams: streamStops, reason: reason ?? null };
    return stopResult;
  }
