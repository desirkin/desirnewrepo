...
const FACTORIES = { KRAKEN_SPOT: createKrakenSpotClient, COINBASE_SPOT: createCoinbaseClient, KRAKEN_DERIVATIVES: createKrakenDerivativesClient, DERIBIT: createDeribitClient, BYBIT: createBybitClient, COINGECKO: createCoinGeckoClient, GECKOTERMINAL: createGeckoTerminalClient, DEFILLAMA: createDefiLlamaClient, COINGLASS: createCoinGlassClient, CRYPTOQUANT: createCryptoQuantClient, SANTIMENT: createSantimentClient, TOKENOMIST: createTokenomistClient, BINANCE_SPOT: createBinanceClient, BITSTAMP_SPOT: createBitstampClient, BINANCE_US_SPOT: createBinanceUsClient };
...
// attic/ (SENSE-CULL-2) and are absent here too.
export const KEPT_MARKET_PROVIDERS = Object.freeze(['KRAKEN_SPOT', 'COINBASE_SPOT', 'KRAKEN_DERIVATIVES', 'DERIBIT', 'BYBIT', 'COINGECKO', 'GECKOTERMINAL', 'DEFILLAMA', 'COINGLASS', 'CRYPTOQUANT', 'SANTIMENT', 'TOKENOMIST', 'BINANCE_SPOT', 'BITSTAMP_SPOT', 'BINANCE_US_SPOT']);
export const MARKET_FACTORY_IDS = Object.freeze(Object.keys(FACTORIES));
export const DEFAULT_SWEEP_FAMILIES = Object.freeze(['SPOT_PRICE_CHART', 'DERIVATIVES_FUNDING_OI', 'OPTIONS_TERM_SKEW', 'SUPPLY_UNLOCKS', 'DEX_DEFI', 'ONCHAIN_ENTITY_FLOW', 'NETWORK_ACTIVITY', 'STABLECOIN_LIQUIDITY', 'ETF_FLOWS', 'MACRO_RELEASES', 'LIQUIDATIONS', 'OFFICIAL_SOCIAL_EVENTS', 'INFRASTRUCTURE_STATUS', 'CROSS_VENUE']);
  ...
  async function acquireInternal(family, coin, options = {}) {
    ...
    switch (family) {
      ...
      case 'NETWORK_ACTIVITY': {
        // the policy-selected entitled paid source fills the requested metrics; never both paid alternatives by default;
        // requested metrics / window are forwarded, never a family-only refresh (the free Coin Metrics route was retired)
