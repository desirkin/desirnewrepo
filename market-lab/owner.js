...
import { createTwelveDataClient, TWELVEDATA_BAR_REFRESH_MS } from './providers/twelvedata.js';
...
const FACTORIES = { KRAKEN_SPOT: createKrakenSpotClient, COINBASE_SPOT: createCoinbaseClient, KRAKEN_DERIVATIVES: createKrakenDerivativesClient, DERIBIT: createDeribitClient, BYBIT: createBybitClient, COINGECKO: createCoinGeckoClient, GECKOTERMINAL: createGeckoTerminalClient, DEFILLAMA: createDefiLlamaClient, COINGLASS: createCoinGlassClient, CRYPTOQUANT: createCryptoQuantClient, SANTIMENT: createSantimentClient, COINMETRICS: createCoinMetricsClient, FRED: createFredClient, TWELVEDATA: createTwelveDataClient, TOKENOMIST: createTokenomistClient, BINANCE_SPOT: createBinanceClient };
...
export function createResearchOwner({ policy, subjects, env = {}, researchRoot = null, mode = 'STANDALONE', clock = () => Date.now(), log = () => {}, transport = null, fetchImpl = globalThis.fetch, WebSocketImpl = globalThis.WebSocket, wsUrls = {}, settledAccessors = {}, socialProjection = null, timers = { setInterval, clearInterval, setTimeout, clearTimeout }, recorder = null, quotaJournal = null, onRecordingFailure = null, closeDrainMs = CLOSE_DRAIN_MS } = {}) {
  ...
  async function acquireInternal(family, coin, options = {}) {
    ...
    switch (family) {
      ...
      case 'CROSS_ASSET': for (const x of subjects.crossAsset) { await run('TWELVEDATA', 'symbol-search', `tds:${x.symbol}`, () => clients.TWELVEDATA.resolve({ symbol: x.symbol, exchange: x.exchange, proxyFor: x.proxyFor, signal }), { ttlMs: DAY_MS }); await run('TWELVEDATA', 'time-series', `tdb:${x.symbol}`, () => clients.TWELVEDATA.bars({ symbol: x.symbol, interval: '1h', outputsize: 100, signal }), { ttlMs: TWELVEDATA_BAR_REFRESH_MS }); } break;
