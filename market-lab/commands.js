...
export async function runCoverage({ policy, subjects, env = {}, out, probe = false, fetchImpl = globalThis.fetch, clock = () => Date.now(), researchRoot = null }) {
  ...
  if (probe) { // policy-authorized metadata / entitlement requests ONLY (catalog / census routes, one call each), every one through the R01 guard as PROBE
    ...
    probeResults = {};
    const probes = { KRAKEN_SPOT: () => owner.clients.KRAKEN_SPOT.loadCatalog(), COINBASE_SPOT: () => owner.clients.COINBASE_SPOT.loadProducts(), KRAKEN_DERIVATIVES: () => owner.clients.KRAKEN_DERIVATIVES.loadInstruments(), COINGECKO: () => owner.clients.COINGECKO.loadCoinsList(), GECKOTERMINAL: () => owner.clients.GECKOTERMINAL.pool({ network: 'eth', address: '0x88e6a0c2ddd26feeb64f039a2c41296fcb3f5640', canonicalCoin: 'ETH' }), DEFILLAMA: () => owner.clients.DEFILLAMA.chains({ names: ['Ethereum'] }), COINGLASS: () => owner.clients.COINGLASS.supportedCoins(), CRYPTOQUANT: () => owner.clients.CRYPTOQUANT.series({ asset: 'btc', canonicalCoin: 'BTC', metricId: 'exchange_reserve', limit: 1 }), TOKENOMIST: () => owner.clients.TOKENOMIST.tokenList({ slugs: ['bitcoin'] }) };
