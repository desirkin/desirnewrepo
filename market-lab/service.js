...
export function createResearchService({ policy, subjects, env = {}, researchRoot, mode = 'STANDALONE', clock = () => Date.now(), log = () => {}, fetchImpl = globalThis.fetch, WebSocketImpl = globalThis.WebSocket, wsUrls = {}, settledAccessors = {}, socialSource = null, timers = { setInterval, clearInterval, setTimeout, clearTimeout }, httpPort = null, httpHost = '127.0.0.1', caseEverySeconds = null, families = null, rotation = null, limits = RESOURCE_DEFAULTS, closeDrainMs = undefined, socratesActivation = null } = {}) {
  ...
  const owner = createResearchOwner({ policy, subjects, env, researchRoot: root, mode, clock, log, fetchImpl, WebSocketImpl, wsUrls, settledAccessors, socialProjection, timers, onRecordingFailure: (rec) => onRecordingFailure(rec), ...(closeDrainMs === undefined ? {} : { closeDrainMs }) }); // P3: one drain bound for both lifecycle owners
  const runtime = createCaseRuntime({ policy, env, owner, clock, log, fetchImpl, budgetDir, contextRebuilder: ownerContextRebuilder(owner, { referenceNotionals: subjects.referenceNotionals ?? [1000, 10000], limits }), socialProjectionOf: socialProjection ? ({ canonicalCoin, asOfTs }) => { try { return socialProjection(canonicalCoin, { asOfTs }); } catch { return null; } } : null, mode: policy.mode, timers: { setTimeout: timers.setTimeout, clearTimeout: timers.clearTimeout }, socratesActivation, ...(closeDrainMs !== undefined ? { closeDrainMs } : {}) });
  ...
  async function start() {
    ...
    try { await owner.start({ outDir: segmentDir, families, rotation, awaitInitialSweep: false }); } catch (err) { state = 'FAILED'; lastError = String(err?.message ?? err).slice(0, 200); throw err; }
    ...
    return { state, http: boundPort ? { host: httpHost, port: boundPort } : null, segmentDir, accountingDir, bootstrap: owner.status().bootstrap };
  }
