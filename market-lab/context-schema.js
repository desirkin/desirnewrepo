...
const obj = (required, optional = {}) => (v, w) => {
  ...
  for (const k of Object.keys(required)) { if (!(k in v)) return `${w}.${k}: required key missing`; const e = required[k](v[k], `${w}.${k}`); if (e) return e; }
  for (const k of Object.keys(v)) { if (k in required) continue; if (!(k in optional)) return `${w}.${k}: undeclared key`; const e = optional[k](v[k], `${w}.${k}`); if (e) return e; }
  return null;
};
...
const numN = nullable(num); const intN = nullable(int); const tsN = nullable(ts); const boolN = nullable(bool); const strN = nullable(str()); const idN = nullable(str(120)); const countN = nullable(count);
...
export function contextSupportError(support, where = 'support') {
  ...
  for (const k of Object.keys(support)) if (!['state', 'reasons', 'attribution'].includes(k)) return `${where}.${k}: undeclared key`;
  ...
  if ('attribution' in support) { const a = str(80)(support.attribution, `${where}.attribution`); if (a) return a; }
  return null;
}
...
const flowSchema = obj({ recipeId: literal('signed_notional'), version, buyNotional: num, sellNotional: num, unknownNotional: num, buyCount: count, sellCount: count, unknownCount: count, signedNotional: numN, knownSideImbalance: numN, knownSideFraction: numN, observedOnly: bool, support: supportOf(FLOW_STATES) });
const statsSchema = obj({ recipeId: literal('trade_size_stats'), version, sizeMedian: numN, sizeP90: numN, interarrivalMeanMs: numN, interarrivalMedianMs: numN, clockPrecisionMs: nullable(en([1, 1000])), support: supportOf(['COMPLETE', 'INSUFFICIENT']) });
const intervalCoverageSchema = obj({ state: en(INTERVAL_COVERAGE_STATES), basis: arr(str(80), 8) });
const tradeWindowSchema = obj({ recipeId: literal('window_ohlcv'), version, startTs: ts, endTs: ts, windowMs: count, count: countN, observedCount: count, volumeBase: numN, quoteNotional: numN, observedNotional: num, vwap: numN, open: numN, high: numN, low: numN, close: numN, coverage: intervalCoverageSchema, range: numN, closeLocation: numN, logReturn: numN, support: supportOf(TRADE_WINDOW_SUPPORT), flow: flowSchema, stats: statsSchema });
...
const triple = obj({ 5: numN, 20: numN, 60: numN });
const indicatorsValue = obj({
  intervalMs: intN, recipeId: literal('indicators'), version, closedBars: count, lastClosedEndTs: tsN, sma: triple, ema: triple, realizedVolatility: triple, atr14: numN,
  rsi14: obj({ value: numN, state: en(['WARMUP', 'FLAT', 'NO_LOSSES', 'NO_GAINS', 'OK']) }), macd: nullable(obj({ macd: num, signal: num, histogram: num })), bollinger20: nullable(obj({ middle: num, upper: num, lower: num, stdev: num })),
  prior20: nullable(obj({ high: num, low: num })), prior60: nullable(obj({ high: num, low: num })), lastClose: numN, support: supportOf(['COMPLETE', 'NO_CLOSED_BARS']), law: literal('WARMUP_IS_NULL_NEVER_ZERO'),
  ...
  provisionalBar: nullable(obj({ periodStartTs: ts, close: num, state: literal('PROVISIONAL') })), completeness: obj({ bars: count, evictedBarsUntilTs: tsN, capReached: bool }), venue: str(80),
});
const bandSide = obj({ qty: num, notional: num, lowerBoundOnly: bool });
...
const spreadFull = obj({ recipeId: literal('spread_bps'), version, mid: num, spreadBps: num, bestBid: num, bestAsk: num, topBidQty: num, topAskQty: num, microprice: numN, bands: obj({ '5bps': band, '10bps': band, '25bps': band }), levelsRetained: obj({ bids: count, asks: count }), support: supportOf(['COMPLETE']), ...bookExtras });
...
const spreadValue = variant((v) => (!('recipeId' in v) ? spreadNoBook : v.mid === null && !('bestBid' in v) ? spreadEmpty : spreadFull));
const walkSchema = obj({ recipeId: literal('book_walk'), version, requested: variant((r) => ('quoteNotional' in r ? obj({ quoteNotional: num }) : obj({ baseQty: num }))), consumedLevels: count, filledBase: num, filledQuote: num, residualQuote: numN, residualBase: numN, averagePrice: numN, worstPrice: numN, coverage: en(['FULL', 'PARTIAL', 'NO_BOOK']), hypothetical: bool }, { preFee: bool });
const roundTripSchema = obj({ recipeId: literal('round_trip_loss'), version, quoteNotional: num, haircut: num, buy: walkSchema, sell: nullable(walkSchema), saleProceeds: numN, lossBps: numN, spreadBps: numN, coverage: en(['FULL', 'PARTIAL_BUY', 'PARTIAL_SELL']), preFee: bool, hypothetical: bool }, { worstBuyPrice: numN, worstSellPrice: numN, scenario: literal('STRESS_NOT_PROBABILITY') });
const roundTripValue = obj({ quoteNotional: num, current: roundTripSchema, previous: nullable(roundTripSchema), lossChangeBps: numN, haircuts: arr(roundTripSchema, 3), buyWalk: walkSchema, sellWalk: walkSchema, fees: literal('NOT_SUPPLIED_PRE_FEE_ONLY'), venue: str(80) });
const pressureSchema = obj({ recipeId: literal('pressure_response_efficiency'), version, responseBps: numN, efficiencyBpsPerMillion: numN, support: supportOf(['NO_BOOK_ENDPOINTS', 'PARTIAL', 'NO_KNOWN_SIDE', 'ZERO_SIGNED_PRESSURE', 'COMPLETE']) }, { signedNotional: numN, spreadStartBps: numN, spreadEndBps: numN, knownSideFraction: numN, alternatives: arr(str(40), 8) });
...
const dispersionSchema = obj({ recipeId: literal('venue_mid_dispersion'), version, medianMid: numN, dispersionBps: numN, n: count, support: supportOf(['COMPLETE', 'INSUFFICIENT_VENUES', 'QUOTE_MISMATCH']) }, { quote: str(20), venues: arr(obj({ venue: str(80), mid: num, receivedTs: tsN }), 64) });
...
const peerValue = obj({ recipeId: literal('peer_relative_move'), version, cohortLabel: str(80), targetReturnBps: numN, medianPeerReturnBps: numN, residualBps: numN, madBps: numN, n: count, positivePeers: countN, excluded: arr(obj({ coin: str(20), reason: str(80) }), 256), support: supportOf(['COMPLETE', 'INSUFFICIENT_PEERS']), cohortMappingKnownAtTs: tsN, cohortNote: str(200) }, { peers: arr(obj({ coin: str(20), returnBps: num }), 256) });
...
const basisValue = obj({ instrument: str(120), marketType: str(40), basisBps: numN, markPrice: numN, indexPrice: numN, openInterest: obj({ value: numN, unit: str(40), changeOverHour: oiChangeSchema }), funding: fundingSchema, ageMs: count, quality: en(QUALITY_STATES) });
const liquidationValue = obj({ recipeId: literal('liquidation_notional_by_side'), version, longNotional: numN, shortNotional: numN, support: supportOf(['COMPLETE', 'COMPLETE_NO_EVENTS', 'UNKNOWN_COVERAGE', 'UNIT_MISMATCH']), venues: arr(str(80), 64) }, { startTs: ts, endTs: ts, events: count, unit: strN, unknownSideNotional: numN, zeroMeaningful: bool, law });
const optionLeg = obj({ instrumentId: str(120), delta: num, deltaDistance: num, markIv: num });
const termEntry = obj({ expiryTs: ts, contracts: count, atm: nullable(obj({ instrumentId: str(120), strike: num, logDistance: num, markIv: num })), call25: nullable(optionLeg), put25: nullable(optionLeg), riskReversal25d: numN, butterfly25d: numN, atmIvBidAskSpread: numN, putCallOiRatio: numN, putCallVolumeRatio: numN, ratioScope: en(['WHOLE_CHAIN', 'ADMITTED_SUBSET']), greeksMissing: count, support: supportOf(['PARTIAL_ADMITTED_SCOPE', 'PARTIAL_CENSUS', 'COMPLETE_ADMITTED_SCOPE']) });
...
const supplyValue = obj({ recipeId: literal('supply_ratios'), version, support: supportOf(['NO_REFERENCE', 'COMPLETE']), reference: obj({ provider: str(40), marketCapUsd: numN, fdvUsd: numN, circulatingSupply: numN, totalSupply: numN, maxSupply: numN, priceUsd: numN, lastUpdatedTs: tsN, ageMs: count }) }, { circulatingOverTotal: numN, circulatingOverMax: numN, fdvOverMarketCap: numN, volumeOverMarketCap: numN, maxSupplyKnown: bool, capMethodologyId: idN });
const unlockValue = obj({ upcoming: arr(obj({ provider: str(40), eventId: str(120), scheduledTs: tsN, timePrecision: en(TIME_PRECISIONS), amountToken: numN, amountUsd: numN, recipientCategory: strN, unlockType: en(UNLOCK_TYPES), tracked: bool, knownAtTs: ts }), 256), untrackedAllocations: count, laws });
const poolValue = obj({ chain: str(80), poolAddress: str(120), dex: idN, baseToken: str(120), quoteToken: str(120), priceUsd: numN, priceQuote: numN, liquidityUsd: numN, volume24hUsd: numN, volume1hUsd: numN, txCount24h: countN, poolCreatedTs: tsN, feePct: numN, ageMs: count, provider: str(40), laws });
...
const netFlowValue = obj({ provider: str(40), recipeId: literal('exchange_net_flow'), version, net: numN, support: supportOf(['MISSING_LEG', 'RECIPE_MISMATCH', 'VALUE_MISSING', 'COMPLETE']), ageMs: countN }, { inflow: num, outflow: num, unit: en(UNITS), entitySet: idN, chain: str(80), window: en(WINDOWS), periodStartTs: ts, periodEndTs: ts, labelVintage: idN, law });
...
const networkValue = obj({ nativeMetric: en(['active_addresses', 'transaction_count', 'transfer_volume', 'fees_total', 'mvrv', 'sopr', 'realized_price']), provider: str(40), ...onchainKeys, previousValue: numN, change: numN, ageMs: count });
...
const pearsonSchema = obj({ recipeId: literal('pearson_correlation'), version, r: numN, n: count, support: supportOf(['INSUFFICIENT_PAIRS', 'ZERO_VARIANCE', 'COMPLETE']) }, { law });
...
const NETWORK_METRICS = ['active_addresses', 'transaction_count', 'transfer_volume', 'network_fees', 'realized_value_metrics'];
