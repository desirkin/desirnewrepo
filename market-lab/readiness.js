...
export function qualifyFamilyEvidence(family, ev, rows, { generatedTs }) {
  ...
  return { qualified: missing.length === 0, complete: missing.length === 0 && ev.complete === true, missing };
}
...
export function qualifyModelDemonstration(m) {
  const missing = [];
  ...
  else { if (!isTs(d.ts)) missing.push('MODEL_DEMONSTRATION_CLOCK_MISSING'); if (typeof d.model !== 'string' || !d.model.length || d.model.length > 120) missing.push('MODEL_DEMONSTRATION_MODEL_MISSING'); if (typeof d.requestId !== 'string' || !d.requestId.length || d.requestId.length > 200) missing.push('MODEL_DEMONSTRATION_REQUEST_MISSING'); if (!isPlain(d.usage) || !isCount(d.usage.inputTokens) || !isCount(d.usage.outputTokens) || d.usage.inputTokens < 1 || d.usage.outputTokens < 1) missing.push('MODEL_DEMONSTRATION_USAGE_MISSING'); }
...
export function liveReadinessManifest({ rows, familyCoverage, modelReadiness = null, generatedTs }) {
  ...
  for (const fam of FAMILIES) {
    ...
    const q = obtained ? qualifyFamilyEvidence(fam, obtained, rows, { generatedTs }) : { qualified: false, complete: false, missing: ['NO_FAMILY_EVIDENCE'] };
  ...
  const model = qualifyModelDemonstration(modelReadiness); const modelVerified = model.qualified;
