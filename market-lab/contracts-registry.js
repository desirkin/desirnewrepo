...
// ---- closed vocabularies ---------------------------------------------------------------------------------
export const FAMILIES = Object.freeze(['SPOT_PRICE_CHART', 'SPOT_FLOW', 'DISPLAYED_LIQUIDITY', 'CROSS_VENUE', 'DERIVATIVES_FUNDING_OI', 'LIQUIDATIONS', 'SUPPLY_UNLOCKS', 'DEX_DEFI', 'ONCHAIN_ENTITY_FLOW', 'NETWORK_ACTIVITY', 'STABLECOIN_LIQUIDITY', 'ETF_FLOWS', 'MACRO_RELEASES', 'OFFICIAL_SOCIAL_EVENTS', 'INFRASTRUCTURE_STATUS']);
export const PAYLOAD_KINDS = Object.freeze(['TRADE', 'BOOK_SNAPSHOT', 'BOOK_COVERAGE', 'CANDLE', 'INSTRUMENT', 'DERIVATIVE_TICK', 'LIQUIDATION', 'ASSET_REFERENCE', 'UNLOCK_EVENT', 'DEX_POOL', 'DEFI_METRIC', 'ONCHAIN_METRIC', 'STABLECOIN_METRIC', 'ETF_FLOW', 'ECONOMIC_EVENT', 'EVENT_REFERENCE', 'PROVIDER_STATUS', 'DERIVATIVE_ANALYTIC_BUCKET', 'L3_BOOK_SNAPSHOT', 'L3_ORDER_EVENT', 'L3_BOOK_COVERAGE']);
...
export const PROVIDER_IDS = Object.freeze(['KRAKEN_SPOT', 'COINBASE_SPOT', 'KRAKEN_DERIVATIVES', 'COINGECKO', 'GECKOTERMINAL', 'DEFILLAMA', 'COINGLASS', 'CRYPTOQUANT', 'SETTLED_RECORDS', 'TOKENOMIST', 'BITSTAMP_SPOT', 'BINANCE_US_SPOT']);
...
// JUDGE-1 §5.2: the four documented large-transfer metrics (NETWORK_ACTIVITY: large transfers WITHOUT entity attribution; the >1m
// group is NESTED inside >100k — never added as independent counts / volume). SENSE-CULL-3 retired their only provider (Santiment);
// the vocabulary is retained decision machinery (the Judge D2 whale input, evidence / context bindings) but is currently UNFED —
// a NETWORK_ACTIVITY sweep resolves them NOT_SUPPORTED. Whether to retire the unfed vocabulary is parked in OPEN-QUESTIONS.md.
...
export const FAMILY_REGISTRY = deepFreeze({
  ...
  DERIVATIVES_FUNDING_OI: { kinds: ['DERIVATIVE_TICK', 'INSTRUMENT'], providers: ['KRAKEN_DERIVATIVES', 'COINGLASS'], metrics: { basis_bps: 'BPS', oi_change: 'CONTRACTS', funding_native: 'NATIVE', dated_basis: 'BPS' } },
  LIQUIDATIONS: { kinds: ['LIQUIDATION'], providers: ['COINGLASS'], metrics: { liquidation_notional_by_side: 'USD' } },
  ...
  ONCHAIN_ENTITY_FLOW: { kinds: ['ONCHAIN_METRIC'], providers: ['CRYPTOQUANT'], metrics: { exchange_net_flow: 'NATIVE', exchange_reserve: 'NATIVE', holder_cohorts: 'NATIVE' } },
  NETWORK_ACTIVITY: { kinds: ['ONCHAIN_METRIC'], providers: ['CRYPTOQUANT'], metrics: { active_addresses: 'COUNT', transaction_count: 'COUNT', transfer_volume: 'NATIVE', network_fees: 'NATIVE', realized_value_metrics: 'RATIO', whale_transaction_count_100k_usd_to_inf: 'COUNT', whale_transaction_count_1m_usd_to_inf: 'COUNT', whale_transaction_volume_100k_usd_to_inf: 'USD', whale_transaction_volume_1m_usd_to_inf: 'USD' } },
  ...
  MACRO_RELEASES: { kinds: ['ECONOMIC_EVENT'], providers: ['COINGLASS'], metrics: { macro_surprise: 'PERCENTAGE_POINTS', release_schedule: 'COUNT' } },
