...
export function createKrakenSpotClient({ transport, clock, log, limits } = {}) {
  ...
  function createStream({ markets, depth = 100, onTrade = () => {}, onBook = () => {}, onCoverage = () => {}, WebSocketImpl, url = null }) {
    ...
    let ws = null; let currentEpoch = null;
    ...
    function onMessage(msg, { receivedTs, epochId, bytesSha256 }) {
      if (msg.method === 'subscribe' && msg.success === false) { const sym = str(obj(msg.result)?.symbol, 40); if (sym && byWs.has(sym)) onCoverage(base.coverage({ endpointId: 'ws-v2', subject: byWs.get(sym).subject, family: 'SPOT_FLOW', state: 'FAILED', reasonCodes: ['PROVIDER_ERROR'], startTs: receivedTs, endTs: receivedTs, epochId })); return; }
    ...
    const remove = (wsname) => { if (!byWs.has(wsname)) return false; ws.send({ method: 'unsubscribe', params: { channel: 'trade', symbol: [wsname] } }); ws.send({ method: 'unsubscribe', params: { channel: 'book', symbol: [wsname], depth } }); byWs.delete(wsname); books.delete(wsname); return true; };
    return { start: () => { base.start(); ws.start(); }, stop: async () => { await ws.stop(); base.stop(); }, status: () => ({ ...ws.status(), ...stats, synced: [...books.values()].filter((b) => b.synced).length, members: [...byWs.keys()] }), books, add, remove };
  }
