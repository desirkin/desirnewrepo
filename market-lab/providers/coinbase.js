...
export function createCoinbaseClient({ transport, clock, log } = {}) {
  ...
  function createStream({ markets, depth = 200, onTrade = () => {}, onBook = () => {}, onCoverage = () => {}, WebSocketImpl, url = null }) {
    ...
    const stats = { trades: 0, bookUpdates: 0, resyncs: 0, unknownSymbols: 0 }; let ws = null; let currentEpoch = null;
    ...
    function onMessage(msg, { receivedTs, epochId, bytesSha256 }) {
      ...
      if (type === 'error') { onCoverage(base.coverage({ endpointId: 'ws-feed', subject: markets[0].subject, family: 'SPOT_FLOW', state: 'FAILED', reasonCodes: ['PROVIDER_ERROR'], startTs: receivedTs, endTs: receivedTs, epochId })); return; }
    ...
    return { start: () => { base.start(); ws.start(); }, stop: async () => { await ws.stop(); base.stop(); }, status: () => ({ ...ws.status(), ...stats, synced: [...books.values()].filter((b) => b.synced).length }), books };
  }
