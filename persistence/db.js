  ...
  async tx(fn) {
    ...
    try {
      ...
      try {
        ...
        // A failed rollback leaves the session's protocol/transaction state
        // unproven even when the primary error was an ordinary constraint or
        // callback failure. Never return that client to the reusable pool.
        // Preserve the primary error for the caller while still applying the
        // shared reachability classifier to a distinct rollback failure.
        if (!connectionFailed) this.#markConnectionFailure(rollbackError);
