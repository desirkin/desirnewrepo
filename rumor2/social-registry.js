...
export const SOCIAL_PROVIDERS = Object.freeze([
  Object.freeze({
    ...
    highPriority: true,
    activation: Object.freeze({ ownerApproval: 'PUBLIC_OFFICIAL_ROUTE', credential: 'NONE', retention: 'SEPARATE_RECEIPT_SETTLEMENT', quota: 'PUBLIC_ROUTE_LIMITS', scope: 'COLLECTION_AND_DID_FILTERS', receipt: 'BLUESKY_RECEIPT_SETTLEMENT' }),
  ...
  Object.freeze({
    ...
    activation: Object.freeze({ ownerApproval: 'ACCOUNT_ATTESTATION', credential: 'NEYNAR_API_KEY', retention: ['CONTENT', 'IDENTITY'], quota: 'MAX_DAILY_REQUESTS', scope: 'QUERY_OR_CATALOG_SCOPE', receipt: 'DURABLE_REQUEST_AND_OBSERVATION' }),
    reason: 'Neynar hosted API (x-api-key) documents event webhooks, cast search, a Kafka stream, and gRPC hub access; Neynar publishes a Free plan with per-endpoint limits (docs, 2026-09-06). This project\'s plan, credits, entitlement, terms (retrieval failed), retention, and actual cost are UNKNOWN/UNVERIFIED. The hub/Snapchain path needs a full node (not lightweight). The normalization mapper exists; live search polling is transport-gated and never opens from key presence alone. Acquisition path selection (search polling vs webhooks) is a proposal pending terms, plan, recovery, and scope.',
  }),
  Object.freeze({
    id: 'X_OFFICIAL',
    providerKind: 'SOCIAL_MICROBLOG',
  ...
  Object.freeze({
    ...
    activation: Object.freeze({ ownerApproval: 'REDDIT_OAUTH_APPROVAL_RECORD', credential: ['REDDIT_CLIENT_ID', 'REDDIT_CLIENT_SECRET'], retention: 'CURRENT_VIEW_ONLY_AFTER_CLASSIFICATION', quota: 'MAX_DAILY_REQUESTS', scope: 'NAMED_SUBREDDITS', receipt: 'ANONYMOUS_REQUEST_RESERVATION' }),
