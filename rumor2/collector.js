...
import { currentReadinessRuntimes, readinessMatrix } from './social-readiness.js';
...
export function startRumor2({
  ...
  function writeStatus() {
    ...
    const status = {
      ...
      socialReadiness: readinessMatrix({ runtimes: { BLUESKY_OFFICIAL: social ? social.status() : { enabled: false, state: 'DARK', gateDetail: 'disabled (RUMOR2_SOCIAL_BLUESKY_ENABLED)' }, X_OFFICIAL: socialX ? socialX.status() : { enabled: false, state: 'DARK', gateDetail: 'disabled (RUMOR2_SOCIAL_X_ENABLED)' }, FARCASTER_OFFICIAL: socialFarcaster ? socialFarcaster.status() : { enabled: false, state: 'DARK' }, ...currentReadinessRuntimes(socialCurrent ? socialCurrent.status() : null) }, knownAtTs: t }),
