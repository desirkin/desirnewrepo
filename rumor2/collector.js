...
import { PROVIDERS, PROVIDER_IDS, normalizeHttpContact, userAgentFor } from './registry.js';
...
import { createSocialCurrentRuntime, currentReadinessRuntimes } from './social-current-runtime.js';
...
export function startRumor2({
  ...
  // Optional DATA-ONLY seam for official-claim symbol resolution. The
  // normal trading composition keeps its legacy permission registry; the
  // isolated collector can instead follow the accepted Kraken catalog
  // without granting those assets any trading eligibility.
  ...
  let registry = buildCoinRegistry(config.universe); // default/trading composition remains unchanged
  let officialCatalogContentId = null;
  const officialRegistry = () => {
    if (!officialCatalogSource) return registry;
    try {
      const snapshot = officialCatalogSource();
      const catalog = snapshot?.catalog;
      if (snapshot?.status === 'ACCEPTED' && typeof catalog?.contentId === 'string' && Array.isArray(catalog?.markets) && catalog.markets.length) {
        if (catalog.contentId !== officialCatalogContentId) {
          registry = buildCoinRegistry([...new Set(catalog.markets.map((market) => market?.base).filter((base) => typeof base === 'string' && base.length))]);
          officialCatalogContentId = catalog.contentId;
        }
      }
    } catch {
      // Retain the last accepted registry; never fall back to a new invented set.
    }
    return registry;
  };
  ...
  const socialResearchStatus = (t) => {
    ...
    return {
      ...
      legacyPermission: officialCatalogSource
        ? { count: officialRegistry().tickers.size, label: 'DATA_ONLY_OFFICIAL_CLAIM_CATALOG (accepted Kraken catalog; no trading permission) — not the deep set' }
        : { count: Array.isArray(config.universe) ? config.universe.length : null, label: 'LEGACY_PERMISSION_SET (config.universe: cost/ledger/official claim registry) — not the discovery count' },
  ...
  function providerGate(p) {
    const hasContact = normalizeHttpContact(contact) !== null;
  ...
  function writeStatus() {
    ...
    const status = {
      ...
      socialReadiness: readinessMatrix({ runtimes: { BLUESKY_OFFICIAL: social ? social.status() : { enabled: false, state: 'DARK', gateDetail: 'disabled (RUMOR2_SOCIAL_BLUESKY_ENABLED)' }, X_OFFICIAL: socialX ? socialX.status() : { enabled: false, state: 'DARK', gateDetail: 'disabled (RUMOR2_SOCIAL_X_ENABLED)' }, FARCASTER_OFFICIAL: socialFarcaster ? socialFarcaster.status() : { enabled: false, state: 'DARK', transportImplemented: true }, ...currentReadinessRuntimes(socialCurrent ? socialCurrent.status() : null) }, knownAtTs: t }),
