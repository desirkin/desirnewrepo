...
export function createSocialRuntime({
  ...
  async function reconcileScope({ fenceHeld, append, lookup }) {
    ...
    // Catalog content and freshness are GLOBAL operational facts shared by all
    // provider-scoped ears. Every identity is checked at the durable door
    // immediately before preparation. A fact another ear already committed is
    // canonically adopted; it is never re-appended with this ear's later clock.
    // This also closes the same race for a newly changed catalog-content row.
    const durableIdentityExists = async (type, sourceEventId) => {
      if (typeof lookup !== 'function') return { ok: false, reason: 'DURABLE_LOOKUP_REQUIRED' };
      const found = await lookup(type, [sourceEventId]);
      if (!found?.ok || !(found.existing instanceof Set || Array.isArray(found.existing))) return { ok: false, reason: found?.reason ?? 'DURABLE_LOOKUP_UNAVAILABLE' };
      return { ok: true, exists: found.existing instanceof Set ? found.existing.has(sourceEventId) : found.existing.includes(sourceEventId) };
    };
    // a PREPARED operation is retried first, byte-identically — a changed candidate never overwrites it
    // A prepared retry normally preserves the exact bytes. The one lawful
    // exception is a shared catalog fact that another provider committed
    // after this operation's prior whole-batch failure (or ambiguous ack):
    // re-check its identity and canonically adopt it before retrying the
    // provider-unique remainder. No truth is rewritten or discarded.
    if (pendingScopeOp) {
      const shared = pendingScopeOp.events.find((event) => event.type === SOCIAL_CATALOG_EVENT_TYPE || event.type === SOCIAL_CATALOG_VERIFIED_EVENT_TYPE);
      if (shared) {
        const existing = await durableIdentityExists(shared.type, shared.sourceEventId);
        if (!existing.ok) { lastError = existing.reason; return { ok: false, reason: existing.reason, scopeOpPending: `${pendingScopeOp.kind}_LOOKUP` }; }
        if (existing.exists && pendingScopeOp.kind === 'VERIFY') {
          const op = pendingScopeOp;
          pendingScopeOp = null;
          lastVerified = { contentId: op.adopt.contentId, observedTs: op.adopt.observedTs, knownAtTs: op.knownAtTs };
          stats.scopeOpRetries += 1;
          stats.scopeVerifications += 1;
          lastError = null;
          return { ok: true, settled: 0, appended: 0, events: [], scopeVerified: true, scopeVerifiedExisting: true };
        }
        if (existing.exists && pendingScopeOp.kind === 'ACTIVATE') {
          knownCatalogIds.add(shared.contentId);
          pendingScopeOp = { ...pendingScopeOp, events: pendingScopeOp.events.filter((event) => event !== shared) };
        }
      }
    ...
    if (activeScope && activeScope.filterId === scope.filterId) {
      ...
      if (cand.catalog && cand.catalog.contentId === activeScope.catalogContentId) {
        ...
        if (cand.catalog.observedTs > seenObserved) {
          ...
          const existing = await durableIdentityExists(ev.type, ev.sourceEventId);
          if (!existing.ok) { lastError = existing.reason; return { ok: false, reason: existing.reason, scopeOpPending: 'VERIFY_LOOKUP' }; }
          if (existing.exists) {
            lastVerified = { contentId: ev.contentId, observedTs: ev.observedTs, knownAtTs };
            stats.scopeVerifications += 1;
            lastError = null;
            return { ok: true, settled: 0, appended: 0, events: [], scopeVerified: true, scopeVerifiedExisting: true };
          }
    ...
    const events = [];
    if (cand.catalog && !knownCatalogIds.has(cand.catalog.contentId)) {
      const catalogEvent = socialCatalogEvent({ catalog: cand.catalog, acceptedKnownAtTs: knownAtTs });
      const existing = await durableIdentityExists(catalogEvent.type, catalogEvent.sourceEventId);
      if (!existing.ok) { lastError = existing.reason; return withCommitted({ ok: false, reason: existing.reason, scopeOpPending: 'CATALOG_LOOKUP' }); }
      if (existing.exists) knownCatalogIds.add(cand.catalog.contentId);
      else events.push(catalogEvent);
    }
