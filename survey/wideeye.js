...
import { appendJsonl, atomicWriteJson, readJsonlTail } from '../lib/jsonl.js';
...
export const WIDEEYE_ARCHIVE_INTERVAL_SEC = 900;
export const WIDEEYE_ARCHIVE_MAX_BYTES = 512 * 1024 * 1024;
...
const populationsFile = () => path.join(surveyDir(), 'populations.jsonl');
...
export function startWideEye({
  ...
  // The normal fly composition may nominate for a later tape session. The
  // isolated data-only composition sets this false so collection cannot
  ...
  try {
    const tail = readJsonlTail(populationsFile(), { maxBytes: 2 * 1024 * 1024 });
    if (tail.torn) archiveError = 'population archive has a torn final line; new archive writes withheld';
    const lastLine = tail.lines.at(-1);
    if (lastLine) {
      const last = JSON.parse(lastLine);
      if (last?.version === SWEEP_POPULATION_VERSION && Number.isSafeInteger(last.tsMs)) {
        lastArchiveTs = last.tsMs;
        archiveOrdinal = Number.isSafeInteger(last.snapshotOrdinal) ? last.snapshotOrdinal : 0;
        savedObservationCount = Number.isSafeInteger(last.savedObservationsTotal) ? last.savedObservationsTotal : 0;
      }
    }
  } catch (err) {
    archiveError = `population archive unreadable: ${String(err.message).slice(0, 160)}`;
  }
  ...
  function sweep(tickers) {
    ...
    let deep = new Set();
    try {
      const candidate = deepCoinsSource();
      deep = candidate instanceof Set ? new Set(candidate) : new Set(Array.isArray(candidate) ? candidate : []);
    } catch {
      deep = new Set();
    }
    let ripples = 0;
    ...
    for (const [key, coin] of keyToCoin) {
      ...
      if (verdict === 'RIPPLE') {
        ripples++;
        ripplesToday++;
        if (nominationEnabled && !deep.has(coin) && Number.isFinite(usd24h) && usd24h >= cfg.nominationFloorUsd) {
    ...
    if (rows.length > 0 && !archiveError && (lastArchiveTs === null || nowMs - lastArchiveTs >= WIDEEYE_ARCHIVE_INTERVAL_SEC * 1000)) {
      try {
        const file = populationsFile();
        if (existsSync(file) && statSync(file).size >= WIDEEYE_ARCHIVE_MAX_BYTES) {
          archiveError = `population archive reached ${WIDEEYE_ARCHIVE_MAX_BYTES} byte retention cap`;
        } else {
          const nextOrdinal = archiveOrdinal + 1;
          const nextTotal = savedObservationCount + rows.length;
          appendJsonl(file, { ...population, snapshotOrdinal: nextOrdinal, savedObservationsTotal: nextTotal, archiveIntervalSec: WIDEEYE_ARCHIVE_INTERVAL_SEC, authority: 'NONE' }, { sync: true });
          lastArchiveTs = nowMs;
          archiveOrdinal = nextOrdinal;
          savedObservationCount = nextTotal;
        }
      } catch (err) {
        archiveError = `population archive write failed: ${String(err.message).slice(0, 160)}`;
      }
    }
    atomicWriteJson(statusFile(), {
      ...
      archive: { intervalSec: WIDEEYE_ARCHIVE_INTERVAL_SEC, maxBytes: WIDEEYE_ARCHIVE_MAX_BYTES, lastArchiveTs, snapshots: archiveOrdinal, savedObservations: savedObservationCount, error: archiveError },
    });
    return ripples;
  }
  ...
  logEvent('WIDEEYE_STARTED', { sweepSec: cfg.sweepSec, catalogRefreshSec: resource.refreshSec, catalogMaxAgeSec: resource.maxAgeSec, catalogMaxMarkets: resource.maxMarkets, nominationEnabled });
  const stop = () => {
    stopping = true;
